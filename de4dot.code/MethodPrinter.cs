...
public class MethodPrinter {
	readonly Dictionary<Instruction, ExInfo> exInfos = new();
	readonly Dictionary<Instruction, string> labels = new();
	readonly Dictionary<Instruction, bool> targets = new();
	IList<ExceptionHandler> allExceptionHandlers;
	IList<Instruction> allInstructions;
	ExInfo lastExInfo;
	LoggerEvent loggerEvent;

	public void Print(LoggerEvent loggerEvent, IList<Instruction> allInstructions,
		IList<ExceptionHandler> allExceptionHandlers) {
		try {
	...
	void Print() {
		...
		foreach (var instr in allInstructions) {
			...
			if (exInfos.TryGetValue(instr, out var exInfo))
				PrintExInfo(exInfo);
			string instrString = instr.OpCode.Name;
			string operandString = GetOperandString(instr);
			var memberRef = instr.Operand as ITokenOperand;
			if (operandString == "")
				Logger.Log(loggerEvent, "{0}", instrString);
			else if (memberRef != null)
				Logger.Log(loggerEvent, "{0,-9} {1} // {2:X8}", instrString, Utils.RemoveNewlines(operandString),
	...
	string GetOperandString(Instruction instr) {
		...
		if (instr.Operand is Parameter arg) {
			string s = InstructionPrinter.GetOperandString(instr);
			if (s != "")
				return s;
	...
	class ExInfo {
		public readonly List<ExceptionHandler> filterStarts = new();
		public readonly List<ExceptionHandler> handlerEnds = new();
		public readonly List<ExceptionHandler> handlerStarts = new();
		public readonly List<ExceptionHandler> tryEnds = new();
		public readonly List<ExceptionHandler> tryStarts = new();
	}
}
