...
public class VariableValues {
	readonly IList<Block> allBlocks;
	readonly IList<Local> locals;
	readonly Dictionary<Local, Variable> variableToValue = new();
...
public abstract class MethodReturnValueInliner {
	...
	public bool UseUnknownArgs { get; set; }
	...
	bool GetLocalVariableValue(Local variable, out object value) {
		if (variableValues == null)
			variableValues = new VariableValues(Method.Body.Variables, allBlocks);
	...
	object FixIntArg(TypeSig type, long value) {
		...
		case ElementType.I8: return value;
	...
	bool GetArg(IMethod method, Block block, ref object arg, ref int instrIndex) {
		while (true) {
			...
			case Code.Ldloc_2:
			case Code.Ldloc_3:
				GetLocalVariableValue(instr.Instruction.GetLocal(Method.Body.Variables), out arg);
				break;
