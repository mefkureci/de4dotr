...
public class Modules : IResolver {
	readonly List<MTypeDef> allTypes = new();
	readonly AssemblyHash assemblyHash = new();
	readonly List<MTypeDef> baseTypes = new();
	readonly IDeobfuscatorContext deobfuscatorContext;
	readonly List<Module> modules = new();
	readonly Dictionary<ModuleDef, Module> modulesDict = new();

	readonly AssemblyKeyDictionary<MTypeDef> typeToTypeDefDict = new();
	...
	class AssemblyHash {
		readonly IDictionary<string, ModuleHash> assemblyHash =
	...
	class ModuleHash {
		readonly ModulesDict modulesDict = new();
		Module mainModule;
	...
	class ModulesDict {
		readonly IDictionary<string, Module> modulesDict = new Dictionary<string, Module>(StringComparer.Ordinal);

		public IEnumerable<Module> Modules => modulesDict.Values;

		public void Add(Module module) {
			string moduleName = module.ModuleDefMD.Name.String;
	...
	class AssemblyKeyDictionary<T> where T : class {
		readonly Dictionary<ITypeDefOrRef, T> dict =
			new(new TypeEqualityComparer(SigComparerOptions.CompareAssemblyVersion));

		readonly Dictionary<ITypeDefOrRef, List<ITypeDefOrRef>> refs = new(TypeEqualityComparer.Instance);
