...
public class MethodInstances {
	readonly Dictionary<IMethodDefOrRef, List<MethodInst>> methodInstances =
...
public class InterfaceMethodInfos {
	readonly Dictionary<ITypeDefOrRef, InterfaceMethodInfo> interfaceMethods = new(TypeEqualityComparer.Instance);
...
public class MTypeDef : Ref {
	readonly Dictionary<TypeInfo, bool> allImplementedInterfaces = new();
	readonly EventDefDict events = new();
	readonly FieldDefDict fields = new();
	readonly List<MGenericParamDef> genericParams;
	readonly InterfaceMethodInfos interfaceMethodInfos = new();
	readonly MethodDefDict methods = new();
	readonly PropertyDefDict properties = new();
	readonly TypeDefDict types = new();
	readonly MethodInstances virtualMethodInstances = new();
	internal TypeInfo baseType;
	internal IList<MTypeDef> derivedTypes = new List<MTypeDef>();

	bool initializeVirtualMembersCalled;
	...
	void InitializeInterfaceMethods(MethodNameGroups groups) {
			...
			foreach (var ifaceInfo in interfaces) {
				foreach (var methodsList in ifaceInfo.typeDef.virtualMethodInstances.GetMethods()) {
					...
					if (!ifaceMethod.IsVirtual())
						continue;
					var ifaceMethodRef = GenericArgsSubstitutor.Create(methodInst.methodRef,
		...
		foreach (var classMethod in methods.GetValues()) {
			...
			foreach (var overrideMethod in classMethod.MethodDef.Overrides) {
				...
				interfaceMethodInfos.AddMethod(overrideMethod.MethodDeclaration.DeclaringType, ifaceMethod,
					classMethod);
			}
		}
	...
	void InstantiateVirtualMembers(MethodNameGroups groups) {
		if (!TypeDef.IsInterface) {
			if (baseType != null)
				virtualMethodInstances.InitializeFrom(baseType.typeDef.virtualMethodInstances,
