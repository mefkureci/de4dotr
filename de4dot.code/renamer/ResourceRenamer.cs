...
public class ResourceRenamer {
	...
	public void Rename(List<TypeInfo> renamedTypes) {
		...
		renamedTypes.Sort((a, b) => {
			string aesc = EscapeTypeName(a.oldFullName);
			string besc = EscapeTypeName(b.oldFullName);
		...
		nameToResource =
			new Dictionary<string, Resource>(module.ModuleDefMD.Resources.Count * 3, StringComparer.Ordinal);
		...
		foreach (var method in module.GetAllMethods()) {
			...
			for (int i = 0; i < instrs.Count; i++) {
				...
				if (instr.OpCode != OpCodes.Ldstr)
					continue;
				string codeString = (string)instr.Operand;
				...
				if (!oldNameToTypeInfo.TryGetValue(codeString, out var typeInfo))
					continue;
				string newName = EscapeTypeName(typeInfo.type.TypeDef.FullName);

				bool renameCodeString = module.ObfuscatedFile.RenameResourcesInCode ||
				                        IsCallingResourceManagerCtor(instrs, i, typeInfo);
				if (!renameCodeString)
					Logger.v("Possible resource name in code: '{0}' => '{1}' in method {2}",
						Utils.RemoveNewlines(codeString), newName, Utils.RemoveNewlines(method));
				else {
					instr.Operand = newName;
					Logger.v("Renamed resource string in code: '{0}' => '{1}' ({2})", Utils.RemoveNewlines(codeString),
						newName, Utils.RemoveNewlines(method));
				}
			}
		}
	}
	...
	void RenameResources(List<TypeInfo> renamedTypes) {
		...
		foreach (var info in renamedTypes) {
			string oldFullName = EscapeTypeName(info.oldFullName);
			if (!nameToResource.TryGetValue(oldFullName, out var resource))
				continue;
			if (newNames.ContainsKey(resource))
				continue;
			string newTypeName = EscapeTypeName(info.type.TypeDef.FullName);
			string newName = newTypeName + resource.Name.String.Substring(oldFullName.Length);
