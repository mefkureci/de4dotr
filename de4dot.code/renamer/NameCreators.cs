...
public class GenericParamNameCreator : NameCreatorCounter {
	static readonly string[] names = new[] { "T", "U", "V", "W", "X", "Y", "Z" };
...
public class NameCreator : NameCreatorCounter {
	...
	public NameCreator Clone() => new(prefix, num);
...
public class NameInfos {
	readonly IList<NameInfo> nameInfos = new List<NameInfo>();
...
public class TypeNameCreator : ITypeNameCreator {
	readonly NameCreator createClassName;
	readonly NameCreator createDelegateName;
	readonly NameCreator createEnumName;
	readonly NameCreator createInterfaceName;
	readonly NameCreator createStructName;
	readonly NameCreator createUnknownTypeName;
	readonly ExistingNames existingNames;
	readonly NameInfos nameInfos = new();
	...
	public TypeNameCreator(ExistingNames existingNames) {
		...
		string[] names = new[] { "Exception", "EventArgs", "Attribute", "Form", "Dialog", "Control", "Stream" };
	...
	protected virtual NameCreator CreateNameCreator(string prefix) => new(prefix);
	...
	NameCreator GetNameCreator(TypeDef typeDef, string newBaseTypeName) {
		...
		if (typeDef.IsEnum)
			...
			if (typeDef.BaseType != null) {
				string fn = typeDef.BaseType.FullName;
