...
public class TypeInfo : MemberInfo {
	...
	string GetPinvokeName(MMethodDef methodDef) {
		string entryPoint = methodDef.MethodDef.ImplMap.Name.String;
		...
		foreach (var methodDef in type.AllMethods) {
			...
			for (int i = 2; i < instructions.Count; i++) {
				...
				if (ldstr.OpCode.Code != Code.Ldstr)
					continue;
				string fieldName = ldstr.Operand as string;
	...
	void InitVbEventHandlers(FieldDefAndDeclaringTypeDict<MFieldDef> ourFields,
	...
	static bool FindEventCall(IList<Instruction> instructions, ref int index, out IField field,
	...
	void InitFieldEventHandlers(FieldDefAndDeclaringTypeDict<MFieldDef> ourFields,
		MethodDefAndDeclaringTypeDict<MMethodDef> ourMethods) {
		var checker = NameChecker;
		...
		foreach (var methodDef in type.AllMethods) {
			...
			for (int i = 0; i < instructions.Count - 6; i++) {
				...
				string eventName = addHandler.Name.String.Substring(4);
				if (!checker.IsValidEventName(eventName))
					continue;
	...
	void InitTypeEventHandlers(FieldDefAndDeclaringTypeDict<MFieldDef> ourFields,
		MethodDefAndDeclaringTypeDict<MMethodDef> ourMethods) {
		var checker = NameChecker;
		...
		foreach (var methodDef in type.AllMethods) {
			...
			for (int i = 0; i < instructions.Count - 5; i++) {
				...
				string eventName = addMethod.Name.String.Substring(4);
				if (!checker.IsValidEventName(eventName))
					continue;
	...
	string FindWindowsFormsClassName(MTypeDef type) {
		foreach (var methodDef in type.AllMethods) {
			...
			for (int i = 2; i < instructions.Count; i++) {
				...
				if (ldstr.OpCode.Code != Code.Ldstr)
					continue;
				string className = ldstr.Operand as string;
				if (className == null)
					continue;
