...
public class Renamer {
	static readonly string[] delegateClasses = new[] { "System.Delegate", "System.MulticastDelegate" };

	static readonly Regex removeGenericsArityRegex = new(@"`[0-9]+");
	readonly DerivedFrom isDelegateClass;
	readonly MemberInfos memberInfos = new();
	readonly MergeStateHelper mergeStateHelper;
	...
	void RemoveOneClassNamespaces(Module module) {
		...
		foreach (var typeDef in module.GetAllTypes()) {
			string ns = typeDef.TypeDef.Namespace.String;
	...
	void RenameGenericParams2(IEnumerable<MGenericParamDef> genericParams) {
		...
		foreach (var param in genericParams) {
			...
			if (isVerbose)
				Logger.v("GenParam: {0} => {1}", Utils.RemoveNewlines(info.oldFullName),
	...
	void RenameMethods2(TypeInfo info) {
		...
		foreach (var methodDef in info.type.AllMethodsSorted) {
			...
			if (isVerbose)
				Logger.v("Method {0} ({1:X8})", Utils.RemoveNewlines(methodInfo.oldFullName),
			...
			if (RenameMethods && methodInfo.GotNewName()) {
				...
				if (isVerbose)
					Logger.v("Name: {0} => {1}", Utils.RemoveNewlines(methodInfo.oldFullName),
				...
				foreach (var param in methodDef.AllParamDefs) {
					...
					if (isVerbose) {
						if (param.IsReturnParameter)
							Logger.v("RetParam: {0} => {1}", Utils.RemoveNewlines(paramInfo.oldName),
								Utils.RemoveNewlines(paramInfo.newName));
						else
							Logger.v("Param ({0}/{1}): {2} => {3}", param.ParameterDef.MethodSigIndex + 1,
								methodDef.MethodDef.MethodSig.GetParamCount(), Utils.RemoveNewlines(paramInfo.oldName),
								Utils.RemoveNewlines(paramInfo.newName));
					}
				}
			}
	...
	void CreateProperty(MPropertyDef propDef, MMethodDef methodDef, string overridePrefix) {
		...
		string newPropertyName = overridePrefix + propDef.PropertyDef.Name;
	...
	void RestorePropertiesFromNames2(IEnumerable<MethodNameGroup> allGroups) {
		...
		foreach (var group in allGroups) {
			...
			string methodName = groupMethod.MethodDef.Name.String;
			...
			if (Utils.StartsWith(methodName, "get_", StringComparison.Ordinal)) {
				...
				foreach (var method in group.Methods) {
					if (onlyRenamableMethods &&
					    !memberInfos.Type(method.Owner).NameChecker.IsValidPropertyName(propName))
						continue;
		...
		foreach (var type in modules.AllTypes) {
			foreach (var method in type.AllMethodsSorted) {
				...
				if (method.Property != null)
					continue;
				string methodName = method.MethodDef.Name.String;
				if (Utils.StartsWith(methodName, "get_", StringComparison.Ordinal))
	...
	void CreateEvent(MEventDef eventDef, MMethodDef methodDef, EventMethodType methodType, string overridePrefix) {
		...
		string newEventName = overridePrefix + eventDef.EventDef.Name;
		switch (methodType) {
	...
	void RestoreEventsFromNames2(IEnumerable<MethodNameGroup> allGroups) {
		...
		foreach (var group in allGroups) {
			...
			if (Utils.StartsWith(methodName, "add_", StringComparison.Ordinal)) {
				string eventName = methodName.Substring(4);
				foreach (var method in group.Methods) {
				...
				string eventName = methodName.Substring(7);
				foreach (var method in group.Methods) {
					if (onlyRenamableMethods && !memberInfos.Type(method.Owner).NameChecker.IsValidEventName(eventName))
						continue;
	...
	void PrepareRenameMemberDefs(MethodNameGroups groups) {
		...
		foreach (var group in GetSorted(groups)) {
			if (group.HasNonRenamableMethod())
				continue;
			if (group.HasGetterOrSetterPropertyMethod() &&
	...
	void RestoreMethodArgs(MethodNameGroups groups) {
		foreach (var group in groups.GetAllGroups()) {
			...
			string[] argNames = GetValidArgNames(group);
	...
	string[] GetValidArgNames(MethodNameGroup group) {
		...
		foreach (var method in group.Methods) {
			foreach (var ovrd in method.MethodDef.Overrides) {
				...
				if (overrideDef == null) {
					var typeDef = modules.ResolveType(overrideRef.DeclaringType) ??
		...
		string[] argNames = new string[group.Methods[0].ParamDefs.Count];
		foreach (var method in methods) {
			var nameChecker = !method.Owner.HasModule ? null : method.Owner.Module.ObfuscatedFile.NameChecker;
			for (int i = 0; i < argNames.Length; i++) {
				string argName = method.ParamDefs[i].ParameterDef.Name;
	...
	static string GetOverridePrefix(MethodNameGroup group, MMethodDef method) {
		...
		if (overrideMethod.DeclaringType == null)
			return "";
		string name = overrideMethod.DeclaringType.FullName.Replace('/', '.');
	...
	void PrepareRenameEvent(MethodNameGroup group, bool renameOverrides) {
		string eventName =
			PrepareRenameEvent(group, renameOverrides, out string overridePrefix, out string methodPrefix);
		if (eventName == null)
			return;

		string methodName = overridePrefix + methodPrefix + eventName;
		foreach (var method in group.Methods)
			memberInfos.Method(method).Rename(methodName);
	}

	string PrepareRenameEvent(MethodNameGroup group, bool renameOverrides, out string overridePrefix,
		...
		string newEventNameWithPrefix = overridePrefix + newEventName;
		foreach (var method in group.Methods) {
			if (method.Event != null) {
	...
	void PrepareRenameProperty(MethodNameGroup group, bool renameOverrides) {
		string propName = PrepareRenameProperty(group, renameOverrides, out string overridePrefix);
		if (propName == null)
			return;
		...
		string methodName = overridePrefix + methodPrefix + propName;
		foreach (var method in group.Methods)
			memberInfos.Method(method).Rename(methodName);
	}
	...
	string PrepareRenameProperty(MethodNameGroup group, bool renameOverrides, out string overridePrefix) {
		...
		if (propInfo.renamed)
			...
			string propPrefix = GetSuggestedPropertyName(group);
			if (propPrefix == null) {
				trySameName = false;
		...
		string newPropNameWithPrefix = overridePrefix + newPropName;
		foreach (var method in group.Methods) {
			if (method.Property != null) {
	...
	string GetNewPropertyNamePrefix(MethodNameGroup group) {
		...
		string prefix = GetPrefix(propType);
	...
	void PrepareRenameVirtualMethods(MethodNameGroup group, string namePrefix, bool renameOverrides) {
		...
		string overridePrefix = GetOverridePrefix(group, overrideMethod);
		if (renameOverrides && overridePrefix == "")
			return;
		if (!renameOverrides && overridePrefix != "")
			return;
		...
		string newMethodNameWithPrefix = overridePrefix + newMethodName;
		foreach (var method in group.Methods)
	...
	static string GetAvailableName(string prefix, bool tryWithoutZero, MethodNameGroup group,
	...
	class PrepareHelper {
		readonly IEnumerable<MTypeDef> allTypes;
		readonly MemberInfos memberInfos;
		readonly Dictionary<MTypeDef, bool> prepareMethodCalled = new();
	...
	class GroupHelper {
		readonly IEnumerable<MTypeDef> allTypes;
		readonly List<MethodNameGroup> groups = new();
		readonly MemberInfos memberInfos;
		readonly Dictionary<MTypeDef, bool> visited = new();
