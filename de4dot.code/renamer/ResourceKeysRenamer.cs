...
public class ResourceKeysRenamer {
	...
	readonly INameChecker nameChecker;
	readonly Dictionary<string, bool> newNames = new();
	...
	void Rename(TypeDef type, List<RenameInfo> renamed) {
		...
		foreach (var method in type.Methods) {
			...
			for (int i = 0; i < instrs.Count; i++) {
				...
				Logger.v("Renamed resource key {0} => {1}", Utils.ToCsharpString(info.element.Name),
	...
	string GetNewName(ResourceElement elem) {
		...
		string name = CreatePrefixFromStringData((string)stringData.Data);
	...
	string CreatePrefixFromStringData(string data) {
		...
		foreach (string piece in data.Split(' ')) {
			if (piece.Length == 0)
				continue;
			string piece2 = piece.Substring(0, 1).ToUpperInvariant() + piece.Substring(1).ToLowerInvariant();
