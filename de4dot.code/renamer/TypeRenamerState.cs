...
public class TypeRenamerState {
	readonly NameCreator createNamespaceName;
	readonly ExistingNames existingNames;
	readonly Dictionary<string, string> namespaceToNewName;
	...
	public string CreateNamespace(TypeDef type, string ns) {
		...
		// to the same name.
		string key = $" [{type.Module.Location}] [{asmFullName}] [{type.Module.Name}] [{ns}] ";
