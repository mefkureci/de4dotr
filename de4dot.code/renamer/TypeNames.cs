...
public abstract class TypeNames {
	...
	protected Dictionary<string, NameCreator> typeNames = new(StringComparer.Ordinal);
	...
	public string Create(TypeSig typeRef) {
		...
		string typeFullName = typeRef.FullName;
		if (typeNames.TryGetValue(typeFullName, out var nc))
			return nc.Create();

		string fullName = elementType == null ? typeRef.FullName : elementType.FullName;
...
public class VariableNameCreator : TypeNames {
	static readonly Dictionary<string, string> ourFullNameToShortName;
	static readonly Dictionary<string, string> ourFullNameToShortNamePrefix;
...
public class PropertyNameCreator : TypeNames {
	static readonly Dictionary<string, string> ourFullNameToShortName = new(StringComparer.Ordinal);
	static readonly Dictionary<string, string> ourFullNameToShortNamePrefix = new(StringComparer.Ordinal);
