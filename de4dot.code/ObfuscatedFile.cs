...
public class ObfuscatedFile : IObfuscatedFile, IDeobfuscatedFile {
	readonly IAssemblyClientFactory assemblyClientFactory;
	readonly AssemblyModule assemblyModule;
	readonly Options options;

	readonly Dictionary<MethodDef, SimpleDeobFlags> simpleDeobfuscatorFlags = new();
	readonly bool userStringDecrypterMethods;
	...
	void ISimpleDeobfuscator.Deobfuscate(MethodDef method, SimpleDeobfuscatorFlags flags) {
		...
		Deobfuscate(method, "Deobfuscating control flow", blocks => {
			...
			var cflowDeobfuscator = new BlocksCflowDeobfuscator(Deobfuscator.BlocksDeobfuscators, disableNewCFCode);
	...
	void IDeobfuscatedFile.CreateAssemblyFile(byte[] data, string assemblyName, string extension) {
		if (extension == null)
			extension = ".dll";
		string baseDir = Utils.GetDirName(options.NewFilename);
		string newName = Path.Combine(baseDir, assemblyName + extension);
	...
	void IDeobfuscatedFile.SetDeobfuscator(IDeobfuscator deob) => this.Deobfuscator = deob;
	...
	public void Save() {
		...
		assemblyModule.Save(options.NewFilename, mdFlags,
			new PrintNewTokens(ModuleDefMD, Deobfuscator as IModuleWriterListener));
	}
	...
	public void DeobfuscateBegin() {
		...
		case DecrypterType.Emulate:
			...
			if (newProcFactory != null)
				assemblyClient = newProcFactory.Create(AssemblyServiceType.StringDecrypter, ModuleDefMD);
			else
	...
	public void Deobfuscate() {
		...
		for (int i = 0;; i++) {
			...
			if (!Deobfuscator.GetDecryptedModule(i, ref fileData, ref dumpedMethods))
				break;
	...
	void LoadModule(IEnumerable<IDeobfuscator> deobfuscators) {
		var oldModule = ModuleDefMD;
		try {
			ModuleDefMD = assemblyModule.Load();
		}
	...
	bool UnpackNativeImage(IEnumerable<IDeobfuscator> deobfuscators) {
		using (var peImage = new PEImage(Filename)) {
			foreach (var deob in deobfuscators) {
				...
				var oldModule = ModuleDefMD;
				try {
					ModuleDefMD = assemblyModule.Load(unpackedData);
				}
				catch {
	...
	void InitializeDeobfuscator() {
		if (options.StringDecrypterType == DecrypterType.Default)
			options.StringDecrypterType = Deobfuscator.DefaultDecrypterType;
		if (options.StringDecrypterType == DecrypterType.Default)
			options.StringDecrypterType = DecrypterType.Static;

		Deobfuscator.Operations = CreateOperations();
	}
	...
	void DetectObfuscator(IEnumerable<IDeobfuscator> deobfuscators) {
		...
		if (Deobfuscator != null) {
			Deobfuscator.Initialize(ModuleDefMD);
			Deobfuscator.DeobfuscatedFile = this;
			Deobfuscator.Detect();
			return;
		}

		foreach (var deob in deobfuscators) {
			deob.Initialize(ModuleDefMD);
		...
		if (options.ForcedObfuscatorType != null) {
			...
			Deobfuscator = DetectObfuscator2(deobfuscators);
	}
	...
	IDeobfuscator DetectObfuscator2(IEnumerable<IDeobfuscator> deobfuscators) {
		...
		int detectVal = 0;
		foreach (var deob in deobfuscators) {
			this.Deobfuscator = deob; // So we can call deob.CanInlineMethods in deobfuscate()
			int val;
			try {
				val = deob.Detect();
			}
	...
	MetadataFlags GetMetadataFlags() {
		var mdFlags = options.MetadataFlags | Deobfuscator.MetadataFlags;

		// Always preserve tokens if it's an unknown obfuscator
		if (Deobfuscator.Type == "un") {
	...
	public void CheckSupportedStringDecrypter(StringFeatures feature) {
		if ((Deobfuscator.StringFeatures & feature) == feature)
			return;
		throw new UserException($"Deobfuscator {Deobfuscator.TypeLong} does not support this string decryption type");
	}
	...
	void ReloadModule(byte[] newModuleData, DumpedMethods dumpedMethods) {
		...
		using (var oldModule = ModuleDefMD) {
			ModuleDefMD = assemblyModule.Reload(newModuleData, CreateDumpedMethodsRestorer(dumpedMethods),
				Deobfuscator as IStringDecrypter);
			Deobfuscator = Deobfuscator.ModuleReloaded(ModuleDefMD);
		}
	...
	void InitAssemblyClient() {
		...
		if (options.StringDecrypterType == DecrypterType.Delegate)
			assemblyClient.StringDecrypterService.SetStringDecrypterType(StringDecrypterType.Delegate);
		else if (options.StringDecrypterType == DecrypterType.Emulate)
			assemblyClient.StringDecrypterService.SetStringDecrypterType(StringDecrypterType.Emulate);
		else
			throw new ApplicationException($"Invalid string decrypter type '{options.StringDecrypterType}'");
	...
	IEnumerable<int> GetMethodTokens() {
		if (!userStringDecrypterMethods)
			return Deobfuscator.GetStringDecrypterMethods();

		var tokens = new List<int>();

		foreach (string val in options.StringDecrypterMethods) {
			string tokenStr = val.Trim();
	...
	IEnumerable<int> FindMethodTokens(string methodDesc) {
		...
		SplitMethodDesc(methodDesc, out string typeString, out string methodName, out string[] argsStrings);
	...
	void DeobfuscateMethods() {
		...
		Deobfuscator.DeobfuscatedFile = null;

		if (!options.ControlFlowDeobfuscation) {
			if (options.KeepObfuscatorTypes || Deobfuscator.Type == "un")
				return;
		}
		...
		var cflowDeobfuscator = new BlocksCflowDeobfuscator(Deobfuscator.BlocksDeobfuscators);
	...
	bool CanOptimizeLocals() =>
		...
		(GetMetadataFlags() & MetadataFlags.PreserveStandAloneSigRids) == 0;
	...
	void Deobfuscate(MethodDef method, BlocksCflowDeobfuscator cflowDeobfuscator, MethodPrinter methodPrinter,
		...
		Deobfuscator.DeobfuscateMethodBegin(blocks);
		if (options.ControlFlowDeobfuscation) {
			cflowDeobfuscator.Initialize(blocks);
			cflowDeobfuscator.Deobfuscate();
		}

		if (Deobfuscator.DeobfuscateOther(blocks) && options.ControlFlowDeobfuscation)
			cflowDeobfuscator.Deobfuscate();
		...
		Deobfuscator.DeobfuscateMethodEnd(blocks);
	...
	void DeobfuscateStrings(Blocks blocks) {
		...
		case DecrypterType.Static:
			Deobfuscator.DeobfuscateStrings(blocks);
			break;
	...
	static bool GetMethodImplOptions(CustomAttribute cattr, ref int value) {
		...
		object arg = cattr.ConstructorArguments[0].Value;
		if (arg is short) {
			value = (short)arg;
			return true;
		}
	...
	class SavedMethodBodies {
		readonly Dictionary<MethodDef, SavedMethodBody> savedMethodBodies = new();
	...
	public class Options {
		...
		public List<string> StringDecrypterMethods { get; }
