...
abstract class DecrypterBase {
	readonly MethodDefAndDeclaringTypeDict<Info> decrypterMethods = new();
	...
	public TypeDef DelegateType { get; private set; }
	...
	public void Find() {
		foreach (var tmp in module.Resources) {
			...
			if (!resource.Name.String.EndsWith(".resources", StringComparison.Ordinal))
				continue;
			SplitTypeName(resource.Name.String.Substring(0, resource.Name.String.Length - 10), out string ns,
				out string name);
	...
	byte[] Decrypt(byte[] encryptedData) {
		...
		byte[] decryptedData = new byte[encryptedData.Length - KEY_LEN];
		byte[] pkt = PublicKey.GetRawData(PublicKeyBase.ToPublicKeyToken(module.Assembly.PublicKey));
		...
		for (int i = 0, j = 0, ki = 0; i < decryptedData.Length; i++) {
			...
			byte tmp = encryptedData[j];
			encryptedData[j] = encryptedData[ki];
			encryptedData[ki] = tmp;
			decryptedData[i] = (byte)(encryptedData[KEY_LEN + i] ^
			                          encryptedData[(encryptedData[j] + encryptedData[ki]) % (KEY_LEN - 1)]);
		}
