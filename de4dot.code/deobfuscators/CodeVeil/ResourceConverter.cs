...
class ResourceConverter {
	readonly MyResourceDataFactory dataCreator;
	readonly ResourceInfo[] infos;
    ...
    public byte[] Convert()
    {
        // Instantiate ResourceElementSet with the required arguments
	...
	ResourceElement Convert(ResourceInfo info) {
		...
		case 4: // char[]
			resourceData = new CharArrayResourceData(
				dataCreator.CreateUserResourceType(CharArrayResourceData.ReflectionTypeName),
				DataReaderUtils.ReadChars(ref reader, info.length)
			);
			break;
		...
		case 19: // Icon
			resourceData = new IconResourceData(
				dataCreator.CreateUserResourceType(IconResourceData.ReflectionTypeName),
				reader.ReadBytes(info.length)
			);
			break;

		case 20: // Image
			resourceData = new ImageResourceData(
				dataCreator.CreateUserResourceType(ImageResourceData.ReflectionTypeName),
				reader.ReadBytes(info.length)
			);
			break;

            case 31: // binary
                resourceData = dataCreator.CreateSerialized(
                reader.ReadBytes(info.length),
                SerializationFormat.BinaryFormatter, // Updated to match the actual API
                dataCreator.CreateUserResourceType("System.Object,System.Runtime")
                );
                break;
            case 31: // binary
                resourceData = dataCreator.CreateSerialized(
...
case 31: // binary
                ...
                break;
				SerializationFormat.BinaryFormat, // Updated to match the actual API
				dataCreator.CreateUserResourceType("System.Object,System.Runtime")
			);
			break;
		...
		return new ResourceElement { Name = info.name, ResourceData = resourceData };
	}
...
class ImageResourceData : UserResourceData {
	...
	public ImageResourceData(UserResourceType type, byte[] data) : base(type) =>
