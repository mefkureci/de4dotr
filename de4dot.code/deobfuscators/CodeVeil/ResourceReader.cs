...
class ResourceReader {
	...
	public string ResourceReaderName { get; private set; }
	...
	public ResourceInfo[] Read() {
		...
		reader.ReadUInt32();
		ResourceReaderName = reader.ReadSerializedString();
		if (Utils.StartsWith(ResourceReaderName, "System.Resources.ResourceReader", StringComparison.Ordinal))
			throw new InvalidDataException("Resource isn't encrypted");
		ResourceSetName = reader.ReadSerializedString();
		...
		for (int i = 0; i < numResources; i++) {
			string resourceName = ReadResourceName(ref reader, encrypted);
		...
		if (encrypted) {
			...
			uint[] encryptedData = new uint[numDwords];
		...
		if (inflateData) {
			byte[] data = dataReader.ReadRemainingBytes();
	...
	static string ReadResourceName(ref DataReader reader, bool encrypted) {
		...
		for (int i = 0; i < len; i++)
			sb.Append(Rol3(DataReaderUtils.ReadChar(ref reader)));
		return sb.ToString();
	}
