...
class AssemblyResolver {
	readonly List<AssemblyInfo> infos = new();
	...
	public EmbeddedResource BundleDataResource { get; private set; }

	public EmbeddedResource BundleXmlFileResource { get; private set; }
	...
	bool FindTypeAndResources() {
		...
		BundleDataResource = bundleDataTmp;
		BundleXmlFileResource = bundleXmlFileTmp;
	...
	void FindEmbeddedAssemblies() {
		byte[] data = BundleDataResource.CreateReader().ToArray();

		var doc = new XmlDocument();
		doc.Load(XmlReader.Create(BundleXmlFileResource.CreateReader().AsStream()));
		...
		foreach (object tmp in manifest.ChildNodes) {
			...
			byte[] assemblyData = DeobUtils.Inflate(data, offset, data.Length - offset, true);
