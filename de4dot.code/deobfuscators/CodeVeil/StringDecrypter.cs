...
class StringDecrypter {
	static readonly string[] requiredFields = new[] {
		"System.Byte[]", "System.Int32", "System.Int32[]", "System.String[]", "System.UInt32[]"
	};
	...
	public StringDecrypter(ModuleDefMD module, MainType mainType, StringDecrypter oldOne) {
		this.module = module;
		this.mainType = mainType;
		Type = Lookup(oldOne.Type, "Could not find string decrypter type");
		...
		DecryptMethod = Lookup(oldOne.DecryptMethod, "Could not find string decrypter method");
	}
	...
	bool CheckType(TypeDef type) {
		...
		DecryptMethod = decrypterMethodTmp;
		return true;
	}
	...
	FieldDef CheckFields(TypeDef type) {
		...
		byte[] data = stringData.InitialValue;
	...
	static uint[] GetKey(MethodDef method) {
		...
		for (int i = 0; i < instrs.Count - 1; i++) {
			...
			i++;
			uint[] key = ArrayFinder.GetInitializedUInt32Array(4, method, ref i);
			if (key == null)
				continue;
	...
	void DecryptStrings(uint[] key) {
		byte[] data = stringDataField.InitialValue;

		uint[] encryptedData = new uint[data.Length / 4];
		...
		byte[] inflated = DeobUtils.Inflate(decryptedData, 0, decryptedData.Length, true);
