...
class TamperDetection {
	...
	void FindTamperDetectionMethods() {
		foreach (var type in Type.NestedTypes) {
	...
	bool CallsMainTypeTamperCheckMethod(MethodDef method) {
		...
		for (int i = 0; i < instructions.Count; i++) {
			var instrs = DotNetUtils.GetInstructions(instructions, i, OpCodes.Ldtoken, OpCodes.Call, OpCodes.Call,
				OpCodes.Ldc_I8, OpCodes.Call);
			if (instrs == null)
				continue;
