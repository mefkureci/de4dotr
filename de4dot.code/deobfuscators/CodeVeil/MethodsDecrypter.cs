...
class MethodsDecrypter {
	...
	static readonly byte[] initializeMethodEnd = new byte[] {
	...
	public bool Decrypt(byte[] fileData, ref DumpedMethods dumpedMethods) {
		...
		using (var peImage = new MyPEImage(fileData)) {
			...
			byte[] methodsData = FindMethodsData(peImage, fileData);
			if (methodsData == null)
				return false;
	...
	int GetStartOffset(MyPEImage peImage) {
		...
		foreach (uint rva in mainType.Rvas) {
			int rvaOffs = (int)peImage.RvaToOffset(rva);
	...
	class DecrypterV5 : Decrypter {
		...
		public override void Initialize(byte[] methodsData) {
			byte[] data = DeobUtils.Inflate(methodsData, true);
			decryptKey = BitConverter.GetBytes(BitConverter.ToUInt32(data, 0));

			byte[] newMethodsData = new byte[data.Length - 4];
