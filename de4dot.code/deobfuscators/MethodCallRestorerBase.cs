...
public class MethodCallRestorerBase {
	readonly MethodDefAndDeclaringTypeDict<NewMethodInfo> oldToNewMethod = new();
	...
	public void CreateGetManifestResourceStream1(MethodDef oldMethod) {
		...
		var newMethod = builder.InstanceMethod("GetManifestResourceStream", assemblyType.TypeDefOrRef, streamType,
			builder.String);
	...
	public void CreateBitmapCtor(MethodDef oldMethod) {
		...
		var typeType = builder.Type("System", "Type", builder.CorLib);
		var newMethod =
			builder.InstanceMethod(".ctor", bitmapType.TypeDefOrRef, builder.Void, typeType, builder.String);
