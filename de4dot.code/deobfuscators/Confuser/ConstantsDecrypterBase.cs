...
abstract class ConstantsDecrypterBase : IVersionProvider {
	...
	readonly FieldDefAndDeclaringTypeDict<bool> fields = new();
	readonly MethodDefAndDeclaringTypeDict<DecrypterInfo> methodToDecrypterInfo = new();
	...
	protected byte[] DecryptConstant_v17_r73740_dynamic(DecrypterInfo info, byte[] encrypted, uint offs, uint key1,
		byte[] key2) {
	...
	protected byte[] DecryptConstant_v17_r73764_native(DecrypterInfo info, byte[] encrypted, uint offs, uint key1,
		byte[] key2) {
		using (var x86Emu = new X86Emulator(fileData))
			return Decrypt(encrypted, key1,
				(magic, i) => (byte)(x86Emu.Emulate((uint)nativeMethod.RVA, magic) ^ key2[i % key2.Length]));
	}
	...
	static byte[] Decrypt(byte[] encrypted, uint key, Func<uint, int, byte> decryptFunc) {
		...
		byte[] decrypted = new byte[reader.ReadInt32() ^ key];
	...
	public object DecryptInt32(MethodDef caller, MethodDef decryptMethod, object[] args) {
		...
		byte[] data = DecryptData(info, caller, args, out byte typeCode);
	...
	public class DecrypterInfo {
		...
		bool InitializeTypeCodes() {
			...
			if (!FindTypeCode(allBlocks, out doubleType, Code.Call,
				    "System.Double System.BitConverter::ToDouble(System.Byte[],System.Int32)"))
				return false;
			if (!FindTypeCode(allBlocks, out singleType, Code.Call,
				    "System.Single System.BitConverter::ToSingle(System.Byte[],System.Int32)"))
				return false;
			if (!FindTypeCode(allBlocks, out int32Type, Code.Call,
				    "System.Int32 System.BitConverter::ToInt32(System.Byte[],System.Int32)"))
				return false;
			if (!FindTypeCode(allBlocks, out int64Type, Code.Call,
				    "System.Int64 System.BitConverter::ToInt64(System.Byte[],System.Int32)"))
				return false;
			if (!FindTypeCode(allBlocks, out stringType, Code.Callvirt,
				    "System.String System.Text.Encoding::GetString(System.Byte[])") &&
			    !FindTypeCode(allBlocks, out stringType, Code.Callvirt,
