...
class StringDecrypter : IVersionProvider {
	static readonly string[] requiredLocals = new[] {
		"System.Byte[]", "System.IO.BinaryReader", "System.Reflection.Assembly"
	};
	...
	static bool FindSafeKey1(MethodDef method, out uint key) {
		var instrs = method.Body.Instructions;
		for (int i = 0; i < instrs.Count; i++) {
			int index = ConfuserUtils.FindCallMethod(instrs, i, Code.Newobj,
	...
	abstract class Decrypter {
		protected readonly StringDecrypter stringDecrypter;
	...
	class Decrypter_v10_r42915 : Decrypter {
		...
		public Decrypter_v10_r42915(StringDecrypter stringDecrypter, int? key) : base(stringDecrypter) =>
			this.key = key;
	...
	class Decrypter_v10_r48832 : Decrypter {
		...
		public override string Decrypt(MethodDef caller, int magic) {
			...
			var instrs = stringDecrypter.Method.Body.Instructions;
			constReader = new PolyConstantsReader(instrs, false);
			int polyIndex = ConfuserUtils.FindCallMethod(instrs, 0, Code.Callvirt,
	...
	class Decrypter_v11_r49299 : Decrypter {
		...
		public override string Decrypt(MethodDef caller, int magic) {
			...
			var constReader = new Arg64ConstantsReader(stringDecrypter.Method.Body.Instructions, false);
		...
		bool FindPolyStartEndIndexes(out int startIndex, out int endIndex) {
			...
			var local = FindLocal(stringDecrypter.Method);
			if (local == null)
				return false;

			if ((endIndex = FindEndIndex(stringDecrypter.Method)) < 0)
				return false;

			if ((startIndex = FindStartIndex(stringDecrypter.Method, endIndex)) < 0)
				return false;
