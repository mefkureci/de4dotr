...
class ResourceDecrypter : IVersionProvider {
	readonly Dictionary<FieldDef, bool> fields = new();
	...
	public MethodDef Handler { get; private set; }
	...
	bool CheckMethod(MethodDef method) {
		...
		simpleDeobfuscator.Deobfuscate(tmpHandler,
			SimpleDeobfuscatorFlags.Force | SimpleDeobfuscatorFlags.DisableConstantsFolderExtraInstrs);
		...
		if (DotNetUtils.CallsMethod(tmpHandler, "System.Object System.AppDomain::GetData(System.String)")) {
			if (!DotNetUtils.CallsMethod(tmpHandler,
	...
	byte[] Decrypt_v14_r55802() {
		...
		byte[] encypted = reader.ReadBytes(reader.ReadInt32());
		if ((encypted.Length & 1) != 0)
			throw new ApplicationException("Invalid resource data length");
		byte[] decrypted = new byte[encypted.Length / 2];
	...
	byte[] Decrypt_v18_r75367() {
		byte[] encrypted = DecryptXor(resource.CreateReader().ToArray());
