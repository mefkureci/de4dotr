...
class Unpacker : IVersionProvider {
	static readonly string[] requiredFields = new[] { "System.String" };

	static readonly string[] requiredEntryPointLocals = new[] {
		"System.Byte[]", "System.IO.BinaryReader", "System.IO.Stream"
	};

	static readonly string[] requiredDecryptLocals_inflate = new[] {
		"System.Byte[]", "System.IO.Compression.DeflateStream"
	};

	static readonly string[] requiredDecryptLocals_7zip = new[] {
		"System.Byte[]", "System.Int64", "System.IO.BinaryReader", "System.Security.Cryptography.CryptoStream",
	...
	public void Find(ISimpleDeobfuscator simpleDeobfuscator, IDeobfuscator deob) {
		...
		if (decryptLocals.Exists("System.IO.MemoryStream")) {
			...
			else if (DotNetUtils.CallsMethod(entryPoint, "System.Void",
	...
	bool FindEntryPointToken(ISimpleDeobfuscator simpleDeobfuscator, MethodDef cctor, MethodDef entryPoint,
		...
		var modPowMethod = DotNetUtils.GetMethod(cctor.DeclaringType, "System.UInt64",
	...
	static EmbeddedAssemblyInfo CreateEmbeddedAssemblyInfo(EmbeddedResource resource, byte[] data) {
		...
		string asmFullName = mod.Assembly != null ? mod.Assembly.FullName : mod.Name.String;
		return new EmbeddedAssemblyInfo(resource, data, asmFullName, mod.Kind);
	}

	byte[] Decrypt(EmbeddedResource resource) {
		byte[] data = resource.CreateReader().ToArray();
		switch (version) {
	...
	byte[] Decrypt_v15_r60785(byte[] data) {
		var reader = new BinaryReader(new MemoryStream(DeobUtils.Inflate(data, true)));
		data = Decrypt_v15_r60785(reader, out byte[] key, out byte[] iv);
