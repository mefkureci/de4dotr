...
public class DeobfuscatorInfo : DeobfuscatorInfoBase {
	...
	readonly BoolOption decryptMainAsm;
	readonly BoolOption removeAntiDebug;
	readonly BoolOption removeAntiDump;
	...
	protected override IEnumerable<Option> GetOptionsInternal() =>
		new List<Option> { removeAntiDebug, removeAntiDump, decryptMainAsm };
}

class Deobfuscator : DeobfuscatorBase, IStringDecrypter {
	const bool useAttributeVersion = true;

	readonly List<EmbeddedAssemblyInfo> embeddedAssemblyInfos = new();
	...
	bool hasInitializedConstantsDecrypter;

	bool hasInitializedConstantsDecrypter15;

	bool hasInitializedConstantsDecrypter17;

	bool hasInitializedStringDecrypter;
	bool hasUnpacked;
	...
	public override IEnumerable<IBlocksDeobfuscator> BlocksDeobfuscators {
		get {
			...
			if (!startedDeobfuscating && int32ValueInliner != null)
				list.Add(new ConstantsInliner(int32ValueInliner, int64ValueInliner, singleValueInliner,
					doubleValueInliner) { ExecuteIfNotModified = true });
	...
	string GetVersionString() {
		var versionProviders = new IVersionProvider[] {
			jitMethodsDecrypter, memoryMethodsDecrypter, proxyCallFixer, antiDebugger, antiDumping, resourceDecrypter,
			constantsDecrypterV18, constantsDecrypterV17, constantsDecrypterV15, stringDecrypter, unpacker
		};
	...
	public override IDeobfuscator ModuleReloaded(ModuleDefMD module) {
		...
		newOne.jitMethodsDecrypter = hasUnpacked
			? new JitMethodsDecrypter(module, DeobfuscatedFile)
		...
		newOne.memoryMethodsDecrypter = hasUnpacked
			? new MemoryMethodsDecrypter(module, DeobfuscatedFile)
	...
	void DumpEmbeddedAssemblies() {
		if (mainAsmInfo != null) {
			...
			string name = (asm == null ? module.Name : asm.Name).String;
	...
	void InitializeStringDecrypter() {
		...
		staticStringInliner.Add(stringDecrypter.Method,
			(method, gim, args) => stringDecrypter.Decrypt(staticStringInliner.Method, (int)args[0]));
	...
	void InitializeConstantsDecrypterV18() {
		...
		foreach (var info in constantsDecrypterV18.Decrypters) {
			staticStringInliner.Add(info.method,
				(method, gim, args) => constantsDecrypterV18.DecryptString(method, gim, (uint)args[0], (ulong)args[1]));
			int32ValueInliner.Add(info.method,
				(method, gim, args) => constantsDecrypterV18.DecryptInt32(method, gim, (uint)args[0], (ulong)args[1]));
			int64ValueInliner.Add(info.method,
				(method, gim, args) => constantsDecrypterV18.DecryptInt64(method, gim, (uint)args[0], (ulong)args[1]));
			singleValueInliner.Add(info.method,
				(method, gim, args) => constantsDecrypterV18.DecryptSingle(method, gim, (uint)args[0], (ulong)args[1]));
			doubleValueInliner.Add(info.method,
				(method, gim, args) => constantsDecrypterV18.DecryptDouble(method, gim, (uint)args[0], (ulong)args[1]));
		}
	...
	void Initialize(ConstantsDecrypterBase constDecrypter, ref bool hasInitialized) {
		...
		foreach (var info in constDecrypter.DecrypterInfos) {
			staticStringInliner.Add(info.decryptMethod,
				(method, gim, args) => constDecrypter.DecryptString(staticStringInliner.Method, method, args));
			int32ValueInliner.Add(info.decryptMethod,
				(method, gim, args) => constDecrypter.DecryptInt32(int32ValueInliner.Method, method, args));
			int64ValueInliner.Add(info.decryptMethod,
				(method, gim, args) => constDecrypter.DecryptInt64(int64ValueInliner.Method, method, args));
			singleValueInliner.Add(info.decryptMethod,
				(method, gim, args) => constDecrypter.DecryptSingle(singleValueInliner.Method, method, args));
			doubleValueInliner.Add(info.decryptMethod,
				(method, gim, args) => constDecrypter.DecryptDouble(doubleValueInliner.Method, method, args));
		}
	...
	void SetConfuserVersion(TypeDef type) {
		string s = DotNetUtils.GetCustomArgAsString(GetModuleAttribute(type) ?? GetAssemblyAttribute(type), 0);
		if (s == null)
			return;
		var val = Regex.Match(s, @"^Confuser v(\d+)\.(\d+)\.(\d+)\.(\d+)$");
