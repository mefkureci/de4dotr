...
class AntiDebugger : IVersionProvider {
	...
	static MethodDef GetAntiDebugMethod(TypeDef type, MethodDef initMethod) {
		foreach (var method in type.Methods) {
			...
			if (!DotNetUtils.IsMethod(method, "System.Void", "()") &&
			    !DotNetUtils.IsMethod(method, "System.Void", "(System.Object)"))
				continue;
	...
	bool CheckMethod_normal(TypeDef type, MethodDef initMethod) {
		...
		if (DotNetUtils.CallsMethod(initMethod,
			...
			int failFastCalls = ConfuserUtils.CountCalls(antiDebugMethod,
			...
			if (!DotNetUtils.CallsMethod(antiDebugMethod,
				    "System.Void System.Threading.Thread::.ctor(System.Threading.ParameterizedThreadStart)")) {
		...
		else if (!DotNetUtils.CallsMethod(initMethod,
