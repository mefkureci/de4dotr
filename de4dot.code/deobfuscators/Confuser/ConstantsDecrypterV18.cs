...
class ConstantsDecrypterV18 : IVersionProvider {
	...
	readonly MethodDefAndDeclaringTypeDict<DecrypterInfo> decrypters = new();
	readonly byte[] fileData;
	readonly ModuleDefMD module;
	readonly ISimpleDeobfuscator simpleDeobfuscator;
	byte[] constants;
	...
	public MethodDef NativeMethod { get; private set; }

	public TypeDef LzmaType { get; private set; }
	...
	public void Find() {
		...
		simpleDeobfuscator.Deobfuscate(cctor,
			SimpleDeobfuscatorFlags.Force | SimpleDeobfuscatorFlags.DisableConstantsFolderExtraInstrs);
		...
		if (FindKeys_v18_r75367(info))
			InitVersion(cctor, ConfuserVersion.v18_r75367_normal, ConfuserVersion.v18_r75367_dynamic,
				ConfuserVersion.v18_r75367_native);
		else if (FindKeys_v18_r75369(info)) {
			LzmaType = ConfuserUtils.FindLzmaType(cctor);
			if (LzmaType == null)
				InitVersion(cctor, ConfuserVersion.v18_r75369_normal, ConfuserVersion.v18_r75369_dynamic,
				...
				InitVersion(cctor, ConfuserVersion.v19_r77172_normal, ConfuserVersion.v19_r77172_dynamic,
					ConfuserVersion.v19_r77172_native);
			else if (DotNetUtils.CallsMethod(method, "System.Void System.Diagnostics.StackFrame::.ctor(System.Int32)"))
				InitVersion(cctor, ConfuserVersion.v19_r78363_normal, ConfuserVersion.v19_r78363_dynamic,
					ConfuserVersion.v19_r78363_native);
			else {
				int index1 = ConfuserUtils.FindCallMethod(cctor.Body.Instructions, 0, Code.Callvirt,
					"System.Reflection.Module System.Reflection.MemberInfo::get_Module()");
				int index2 = ConfuserUtils.FindCallMethod(cctor.Body.Instructions, 0, Code.Callvirt,
					"System.Int32 System.Reflection.MemberInfo::get_MetadataToken()");
				if (index1 < 0 || index2 < 0) {
				}

				if (index2 - index1 == 3)
					InitVersion(cctor, ConfuserVersion.v19_r78056_normal, ConfuserVersion.v19_r78056_dynamic,
						ConfuserVersion.v19_r78056_native);
				else if (index2 - index1 == -4)
					InitVersion(cctor, ConfuserVersion.v19_r79630_normal, ConfuserVersion.v19_r79630_dynamic,
						ConfuserVersion.v19_r79630_native);
			}
		}
		else
			return;
	...
	EmbeddedResource FindResource(uint magic) {
		string name = Encoding.UTF8.GetString(BitConverter.GetBytes(magic));
	...
	static bool FindKey0(MethodDef method, out uint key) {
		...
		for (int i = 0; i < instrs.Count; i++) {
			...
			if (index < 0)
				break;
			int index2 = ConfuserUtils.FindCallMethod(instrs, i, Code.Call,
	...
	static bool IsDecryptMethodSignature(MethodDef method) {
		...
		if (method.Attributes !=
		    (MethodAttributes.Static | MethodAttributes.HideBySig | MethodAttributes.CompilerControlled))
			return false;
	...
	static bool FindKey0d_v18_r75367(DecrypterInfo info) {
		...
		for (int i = 0; i < instrs.Count; i++) {
			...
			if (index < 0)
				break;
			int index2 = ConfuserUtils.FindCallMethod(instrs, index, Code.Call,
				"System.Byte[] System.BitConverter::GetBytes(System.Int32)");
			if (index2 < 0)
				break;
			if (index2 - index != 3)
	...
	byte[] DecryptResource_v18_r75367_normal(byte[] encrypted) {
		byte[] key = GetSigKey();
		byte[] decrypted = ConfuserUtils.Decrypt(BitConverter.ToUInt32(key, 12) * key0, encrypted);
