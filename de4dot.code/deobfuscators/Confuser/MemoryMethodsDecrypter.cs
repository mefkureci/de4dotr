...
class MemoryMethodsDecrypter : MethodsDecrypterBase {
	...
	protected override bool CheckType(TypeDef type, MethodDef initMethod) {
		...
		bool callsFileStreamCtor = DotNetUtils.CallsMethod(initMethod,
		...
		if (!DotNetUtils.HasString(initMethod, "Module error"))
			...
			int calls = ConfuserUtils.CountCalls(initMethod,
	...
	byte[] DecryptMethodsData_v14_r57884(MyPEImage peImage, bool hasStrongNameInfo) {
		...
		reader.Position = 0;
		byte[] md5SumData = reader.ReadBytes((int)peImage.OptionalHeader.CheckSum ^ (int)key0);
		...
		byte[] decrypted = Decrypt(encrypted, iv, md5SumData);
