...
class JitMethodsDecrypter : MethodsDecrypterBase, IStringDecrypter {
	...
	protected override bool CheckType(TypeDef type, MethodDef initMethod) {
		...
		case 39:
			if (!DotNetUtils.CallsMethod(initMethod, "System.Void System.Console::WriteLine(System.Char)")) {
				if (DotNetUtils.CallsMethod(decryptMethod,
			...
			else if (DotNetUtils.CallsMethod(decryptMethod,
				         "System.Security.Cryptography.Rijndael System.Security.Cryptography.Rijndael::Create()"))
	...
	static bool FindKey4(MethodDef method, out uint key) {
		...
		for (int index = 0; index < instrs.Count; index++) {
			index = ConfuserUtils.FindCallMethod(instrs, index, Code.Call,
	...
	bool InitializeMethodDataIndexes_v17_r73477(MethodDef method) {
		...
		var methodDataType = FindFirstThreeIndexes(method, out methodDataIndexes.maxStack, out methodDataIndexes.ehs,
			out methodDataIndexes.options);
	...
	static TypeDef FindFirstThreeIndexes(MethodDef method, out int maxStackIndex, out int ehsIndex,
		out int optionsIndex) {
		var instrs = method.Body.Instructions;
	...
	DumpedMethods Decrypt_v17_r73404(MyPEImage peImage, byte[] fileData) {
		...
		for (uint rid = 1; rid <= methodDef.Rows; rid++) {
			...
			byte[] codeData = DecryptMethodData_v17_r73404(methodsData, mdOffs + 2, (uint)key, len);
	...
	DumpedMethods Decrypt(MyPEImage peImage, byte[] fileData, DecryptMethodData decrypter) {
		...
		for (uint rid = 1; rid <= methodDef.Rows; rid++) {
			...
			decrypter.Decrypt(methodsData, methodDataOffset, (uint)key, len, out uint[] methodData,
				out byte[] codeData);
	...
	byte[] DecryptMethodData_v17_r73404(byte[] fileData, int offset, uint k1, int size) {
		...
		byte[] kbytes = BitConverter.GetBytes(k1);
	...
	abstract class DecryptMethodData {
		public abstract void Decrypt(byte[] fileData, int offset, uint k1, int size, out uint[] methodData,
			out byte[] codeData);
	...
	class DecryptMethodData_v17_r73477 : DecryptMethodData {
		public override void Decrypt(byte[] fileData, int offset, uint k1, int size, out uint[] methodData,
			out byte[] codeData) {
			byte[] data = new byte[size];
			Array.Copy(fileData, offset, data, 0, data.Length);
			byte[] key = BitConverter.GetBytes(k1);
	...
	class DecryptMethodData_v18_r75402 : DecryptMethodData {
		readonly JitMethodsDecrypter jitDecrypter;
