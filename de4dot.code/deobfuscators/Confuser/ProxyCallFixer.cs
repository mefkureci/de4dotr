...
class ProxyCallFixer : ProxyCallFixer2, IVersionProvider, IDisposable {
	...
	readonly MethodDefAndDeclaringTypeDict<ProxyCreatorInfo> methodToInfo = new();
	readonly string ourAsm;
	ushort callvirtChar;
	FieldDefAndDeclaringTypeDict<List<MethodDef>> fieldToMethods = new();
		...
		get {
			...
			foreach (var info in methodToInfo.GetValues()) {
				list.Add(new Tuple<MethodDef, string> {
					Item1 = info.creatorMethod, Item2 = "Delegate creator method"
				});
				list.Add(new Tuple<MethodDef, string> {
					Item1 = info.nativeMethod, Item2 = "Calculate RID native method"
				});
			}

			foreach (var methods in fieldToMethods.GetValues()) {
				foreach (var method in methods) {
					list.Add(new Tuple<MethodDef, string> { Item1 = method, Item2 = "Proxy delegate method" });
				}
			}
	...
	void GetCallInfo_v10_r42915(DelegateInitInfo info, ProxyCreatorInfo creatorInfo, out IMethod calledMethod,
		out OpCode callOpcode) {
	...
	void GetCallInfo_v10_r48717(DelegateInitInfo info, ProxyCreatorInfo creatorInfo, out IMethod calledMethod,
		out OpCode callOpcode) {
		...
		uint token =
			BitConverter.ToUInt32(Encoding.Unicode.GetBytes(info.field.Name.String.ToCharArray(), offs, 2), 0) ^
			creatorInfo.magic;
		uint table = token >> 24;
		...
		bool isCallvirt = false;
		if (creatorInfo.proxyCreatorType == ProxyCreatorType.CallOrCallvirt &&
		    info.field.Name.String[callvirtOffs] == '\r')
			isCallvirt = true;
		callOpcode = GetCallOpCode(creatorInfo, isCallvirt);
	}

	void GetCallInfo_v14_r58857(DelegateInitInfo info, ProxyCreatorInfo creatorInfo, out IMethod calledMethod,
		out OpCode callOpcode) {
		int offs = creatorInfo.proxyCreatorType == ProxyCreatorType.CallOrCallvirt ? 1 : 0;
		byte[] nameInfo = DecryptFieldName(info.field.Name.String);
	...
	void Extract_v17_r73740(ProxyCreatorInfo creatorInfo, byte[] nameInfo, out uint arg, out uint table,
		out bool isCallvirt) {
	...
	void GetCallInfo_v17_r73740_normal(DelegateInitInfo info, ProxyCreatorInfo creatorInfo, out IMethod calledMethod,
		out OpCode callOpcode) {
		byte[] nameInfo = DecryptFieldName(info.field.Name.String);
	...
	void GetCallInfo_v17_r73740_native(DelegateInitInfo info, ProxyCreatorInfo creatorInfo, out IMethod calledMethod,
		out OpCode callOpcode) {
		byte[] nameInfo = DecryptFieldName(info.field.Name.String);
		Extract_v17_r73740(creatorInfo, nameInfo, out uint arg, out uint table, out bool isCallvirt);
		if (x86emu == null)
	...
	void GetCallInfo_v18_r75367_normal(DelegateInitInfo info, ProxyCreatorInfo creatorInfo, out IMethod calledMethod,
		out OpCode callOpcode) =>
		GetCallInfo_v18_r75367(info, creatorInfo, out calledMethod, out callOpcode,
			(creatorInfo2, magic) => creatorInfo2.magic ^ magic);

	void GetCallInfo_v18_r75367_native(DelegateInitInfo info, ProxyCreatorInfo creatorInfo, out IMethod calledMethod,
		out OpCode callOpcode) =>
	...
	void GetCallInfo_v18_r75367(DelegateInitInfo info, ProxyCreatorInfo creatorInfo, out IMethod calledMethod,
		out OpCode callOpcode, Func<ProxyCreatorInfo, uint, uint> getRid) {
		byte[] sig = module.ReadBlob(info.field.MDToken.Raw);
		int len = sig.Length;
	...
	AssemblyRef ReadAssemblyNameReference(BinaryReader reader) {
		...
		string culture = ReadString(reader);
		byte[] pkt = reader.ReadBoolean() ? reader.ReadBytes(8) : null;
		return module.UpdateRowId(new AssemblyRefUser(name, version, pkt == null ? null : new PublicKeyToken(pkt),
			culture));
	}
	...
	public void FindDelegateCreator(ISimpleDeobfuscator simpleDeobfuscator) {
		...
		foreach (var method in type.Methods) {
			...
			if (FindMagic_v14_r58564(method, out uint magic)) {
				if (!DotNetUtils.CallsMethod(method, "System.Byte[] System.Convert::FromBase64String(System.String)")) {
				...
				else if (proxyType != ProxyCreatorType.CallOrCallvirt || !HasFieldReference(method,
					         "System.Reflection.Emit.OpCode System.Reflection.Emit.OpCodes::Castclass"))
					theVersion = ConfuserVersion.v14_r58857;
				else if (proxyType == ProxyCreatorType.CallOrCallvirt && DotNetUtils.CallsMethod(method,
					...
					theVersion = ConfuserVersion.v16_r70489;
			         ...
			         DotNetUtils.CallsMethod(method,
				...
				if ((nativeMethod = FindNativeMethod_v18_r75367(method)) != null)
					theVersion = proxyType != ProxyCreatorType.CallOrCallvirt || callvirtChar == 9
						? ConfuserVersion.v18_r75367_native
						: ConfuserVersion.v18_r75369_native;
				else if (FindMagic_v18_r75367(method, out magic))
					theVersion = proxyType != ProxyCreatorType.CallOrCallvirt || callvirtChar == 9
						? ConfuserVersion.v18_r75367_normal
						: ConfuserVersion.v18_r75369_normal;
					...
					if (proxyType == ProxyCreatorType.CallOrCallvirt &&
					    !DotNetUtils.CallsMethod(method, "System.Int32 System.String::get_Length()"))
						theVersion = ConfuserVersion.v11_r50378;
					int numCalls = ConfuserUtils.CountCalls(method,
			...
			methodToInfo.Add(method,
				new ProxyCreatorInfo(method, proxyType, theVersion, magic, nativeMethod, callvirtChar));
	...
	static bool CommonCheckVersion19(MethodDef method, bool isNormal, int tmpProxyVer, ref ConfuserVersion theVersion) {
		...
		if (tmpProxyVer == 2) {
			if (!CheckCtorProxyType_v19_r78963(method))
				...
				theVersion = isNormal
					? ConfuserVersion.v19_r78963_normal_Newobj
					: ConfuserVersion.v19_r78963_native_Newobj;
			return true;
		}
	...
	static int GetFieldNameIndex(MethodDef method) {
		...
		for (int i = 0; i < instrs.Count; i++) {
			i = ConfuserUtils.FindCallMethod(instrs, i, Code.Callvirt,
				"System.Byte[] System.Text.Encoding::GetBytes(System.Char[],System.Int32,System.Int32)");
			if (i < 0)
				break;
			if (i < 2)
	...
	static bool FindMagic_v17_r73740(MethodDef method, out uint magic) {
		var instrs = method.Body.Instructions;
		for (int i = 0; i < instrs.Count; i++) {
			int index = ConfuserUtils.FindCallMethod(instrs, i, Code.Call,
	...
	static bool Is_v17_r73740(MethodDef method) {
		var instrs = method.Body.Instructions;
		for (int i = 0; i < instrs.Count; i++) {
			int index = ConfuserUtils.FindCallMethod(instrs, i, Code.Callvirt,
				"System.Reflection.MethodBase System.Reflection.Module::ResolveMethod(System.Int32)");
			if (index < 0)
				break;
			if (index < 3)
	...
	static bool FindMagic_v14_r58564(MethodDef method, out uint magic) {
		...
		for (int i = 0; i < instrs.Count; i++) {
			...
			if (index < 0)
				break;
			int index2 = ConfuserUtils.FindCallMethod(instrs, i, Code.Callvirt,
				"System.Reflection.MethodBase System.Reflection.Module::ResolveMethod(System.Int32)");
	...
	static ProxyCreatorType GetProxyCreatorType(MethodDef method, ISimpleDeobfuscator simpleDeobfuscator,
		out int version) {
	...
	FieldDefAndDeclaringTypeDict<DelegateInitInfo> CreateDelegateInitInfos_v10_r42915(MethodDef method) {
		...
		for (int i = 0; i < instrs.Count - 2; i++) {
			...
			if (ldstr.OpCode.Code != Code.Ldstr)
				continue;
			string info = ldstr.Operand as string;
			if (info == null)
				continue;
	...
	static bool FindCallvirtChar(MethodDef method, out ushort callvirtChar) {
		...
		for (int index = 0; index < instrs.Count; index++) {
			index = ConfuserUtils.FindCallMethod(instrs, index, Code.Callvirt,
	...
	class ProxyCreatorInfo {
		...
		public ProxyCreatorInfo(MethodDef creatorMethod, ProxyCreatorType proxyCreatorType, ConfuserVersion version,
			uint magic, MethodDef nativeMethod, ushort callvirtChar) {
