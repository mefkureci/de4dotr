...
public class DeobfuscatorInfo : DeobfuscatorInfoBase {
	...
	readonly BoolOption removeAutomatedErrorReporting;
	readonly BoolOption removeMemoryManager;
	readonly BoolOption removeTamperProtection;

	public DeobfuscatorInfo()
		: base(DEFAULT_REGEX) {
		removeAutomatedErrorReporting =
			new BoolOption(null, MakeArgName("error"), "Remove automated error reporting code", true);
	...
	protected override IEnumerable<Option> GetOptionsInternal() =>
		new List<Option> { removeAutomatedErrorReporting, removeTamperProtection, removeMemoryManager };
}

class Deobfuscator : DeobfuscatorBase {
	readonly Options options;

	readonly IList<StringDecrypterInfo> stringDecrypterInfos = new List<StringDecrypterInfo>();
	readonly IList<StringDecrypter> stringDecrypters = new List<StringDecrypter>();
	Version approxVersion = new(0, 0, 0, 0);
	AssemblyResolver assemblyResolver;
	AssemblyResolverInfo assemblyResolverInfo;
	AutomatedErrorReportingFinder automatedErrorReportingFinder;
	bool canRemoveTypes;
	bool foundSmartAssemblyAttribute;
	bool foundVersion;
	MemoryManagerInfo memoryManagerInfo;
	string obfuscatorName = DeobfuscatorInfo.THE_NAME;
	PointerToLocalFixer pointerToLocalFixer;
	string poweredByAttributeString;
	...
	void FindSmartAssemblyAttributes() {
		foreach (var type in module.Types) {
			if (Utils.StartsWith(type.FullName, "SmartAssembly.Attributes.PoweredByAttribute",
	...
	bool HasEmptyClassesInEveryNamespace() {
		...
		foreach (var type in module.Types) {
			...
			if (!namespaces.ContainsKey(ns))
				namespaces[ns] = 0;
			if (type.Name != "" || type.IsPublic || type.HasFields || type.HasMethods || type.HasProperties ||
			    type.HasEvents)
				continue;
			namespaces[ns]++;
		}
	...
	void InitDecrypters() {
		...
		resourceDecrypterInfo =
			new ResourceDecrypterInfo(module, assemblyResolverInfo.SimpleZipTypeMethod, DeobfuscatedFile);
