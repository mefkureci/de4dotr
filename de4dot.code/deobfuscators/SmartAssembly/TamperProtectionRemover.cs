...
class TamperProtectionRemover {
	...
	readonly List<MethodDef> pinvokeMethods = new();
	...
	bool FindFirstBlocks(Block block, TamperBlocks tamperBlocks, IList<Block> allBlocks, IList<Local> locals) {
		...
		*/
		...
		int start = FindCallMethod(block, index, true,
			calledMethod => calledMethod.ToString() ==
		...
		int index2 = index;
		index = FindCallMethod(block, index, false,
			calledMethod => calledMethod.ToString() == "System.String System.Reflection.Assembly::get_Location()");
		if (index < 0) {
			index = index2;
			index = FindCallMethod(block, index, false,
				calledMethod => calledMethod.ToString() == "System.String System.Reflection.Assembly::get_CodeBase()");
			if (index < 0)
				return false;
		}
		...
		index = FindCallMethod(block, index, false, calledMethod => {
			...
			return DotNetUtils.IsPinvokeMethod(tamperBlocks.pinvokeMethod, "mscorwks",
		...
		if (index < 0) {
			...
			index = FindCallMethod(block, index, false,
				calledMethod => calledMethod.ToString() == "System.String System.Uri::get_LocalPath()");
			if (index < 0) return false;
			index2 = index + 1;
			instr = instrs[--index];
		...
		if (!instrs[index].IsBrfalse()) {
			...
			tamperBlocks.type = Type.V1;
			tamperBlocks.first = new BlockInfo { Block = block, Start = start, End = end };
		}
		else {
		...
		tamperBlocks.second = new BlockInfo { Block = block, Start = start, End = end };
	...
	BlockInfo FindBadBlock(Block last) {
		...
		if (method == null ||
		    method.ToString() != "System.Void System.Security.SecurityException::.ctor(System.String)")
			return null;
		...
		return new BlockInfo { Block = last, Start = start, End = end };
	}
	...
	bool RemoveTamperProtection(Blocks blocks) {
		...
		if (tamperBlocks == null) {
			if (IsTamperProtected(allBlocks))
				Logger.w("Could not remove tamper protection code: {0} ({1:X8})", Utils.RemoveNewlines(blocks.Method),
					blocks.Method.MDToken.ToUInt32());
			return false;
		}
