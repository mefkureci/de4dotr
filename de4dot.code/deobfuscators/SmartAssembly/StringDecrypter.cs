...
class StringDecrypter {
	readonly byte[] decryptedData;
	readonly int stringOffset;
	readonly int xorValue;
	...
	public StringDecrypterInfo StringDecrypterInfo { get; }
	...
	public string Decrypt(int token, int id) {
		...
		case StringDecrypterVersion.V1:
			...
			byte[] buf = Convert.FromBase64String(Encoding.ASCII.GetString(decryptedData, index, len));
			if (buf.Length % 2 != 0)
