...
class ResourceDecrypter {
	readonly ResourceDecrypterInfo resourceDecrypterInfo;
	...
	byte[] Decrypt(byte[] encryptedData) {
		...
		case 1:
			...
			byte[] inflatedBytes = new byte[totalInflatedLength];
			...
			for (int inflateOffset = 0; inflateOffset < totalInflatedLength; inflateOffset += partInflatedLength) {
				...
				int realInflatedLen =
					inflater.Inflate(inflatedBytes, inflateOffset, inflatedBytes.Length - inflateOffset);
