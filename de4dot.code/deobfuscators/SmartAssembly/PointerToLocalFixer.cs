...
class PointerToLocalFixer {
				...
				if (IsStind(instr) || IsLdind(instr)) {
					...
					if (pushes?.Count > 0) {
						...
						if (IsBaseLdlocal(ldlocOrAdd)) {
							...
							int index = instrs.IndexOf(ldlocOrAdd);
				...
				if (IsCallOrNewobj(instr)) {
					...
					for (int j = 0; j < pushes.Count; j++) {
						...
						if (ldloc.OpCode.Code == Code.Add) {
							int index = instrs.IndexOf(ldloc);
							if (index < 2)
								continue;
							var ldci4 = instrs[index - 1];
							ldloc = instrs[index - 2];
	...
	bool IsCallOrNewobj(Instr instr) => instr.OpCode.Code == Code.Call || instr.OpCode.Code == Code.Callvirt ||
