...
class DecrypterType {
	...
	public Resource LinkedResource { get; private set; }

	public MethodDef StringDecrypter1 { get; private set; }

	public MethodDef StringDecrypter2 { get; private set; }

	public IEnumerable<MethodDef> InitMethods => initMethods;

	public IEnumerable<MethodDef> StringDecrypters =>
		new List<MethodDef> { StringDecrypter1, StringDecrypter2 };
	...
	bool Patch2(MyPEImage peImage) {
		...
		for (uint i = 0; i < numPatches; i++, offset += 8) {
			...
			uint value = peImage.OffsetReadUInt32(offset + 4);
