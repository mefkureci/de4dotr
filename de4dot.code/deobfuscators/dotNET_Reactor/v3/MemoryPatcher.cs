...
class MemoryPatcher {
	readonly DecryptMethod decryptMethod = new();
	readonly List<PatchInfo> patchInfos = new();
	...
	void Find(TypeDef type, ICflowDeobfuscator cflowDeobfuscator) {
		var additionalTypes = new List<string> { "System.IO.BinaryWriter" };
	...
	void FindPatchData(TypeDef type, ICflowDeobfuscator cflowDeobfuscator) {
		var locals = new List<string> { "System.Int32[]", "System.UInt32[]" };
		foreach (var method in type.Methods) {
	...
	PatchInfo GetPatchInfo(MethodDef method) {
		...
		int[] ary1 = ArrayFinder.GetInitializedInt32Array(size1, method, ref index1);
		int[] ary2 = ArrayFinder.GetInitializedInt32Array(size2, method, ref index2);
	...
	int[] Decrypt(int[] data) {
		...
		if (decrypted.Length / 4 * 4 != decrypted.Length)
			return null;
		int[] newData = new int[decrypted.Length / 4];
