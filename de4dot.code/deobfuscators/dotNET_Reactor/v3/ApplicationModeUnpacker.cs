...
class IniFile {
	readonly Dictionary<string, string> nameToValue = new(StringComparer.OrdinalIgnoreCase);
...
class ApplicationModeUnpacker {
	static readonly byte[] key1 = new byte[32] {
		0x6B, 0x6C, 0xA7, 0x24, 0x25, 0x37, 0x67, 0x68, 0x4A, 0x2F, 0x28, 0x29, 0x33, 0x77, 0x34, 0x35, 0x5A, 0x5A,
		0x48, 0x57, 0x24, 0x35, 0x24, 0x25, 0x26, 0x67, 0x77, 0x53, 0x41, 0x44, 0x46, 0x32
	};

	static readonly byte[] iv1 = new byte[16] {
		0x73, 0x64, 0xA7, 0x35, 0x24, 0xA7, 0x26, 0x67, 0x34, 0x35, 0x37, 0x21, 0x32, 0x33, 0x6E, 0x6D
	};

	static readonly byte[] key2 = new byte[32] {
		0x28, 0x24, 0x29, 0x28, 0x2F, 0x29, 0x28, 0x29, 0x3D, 0x66, 0x67, 0x35, 0x35, 0x6A, 0x6D, 0x2C, 0xA7, 0x39,
		0x38, 0x2A, 0x6A, 0x67, 0x74, 0x36, 0x35, 0x3D, 0xA7, 0x43, 0x33, 0x33, 0x24, 0x74
	};

	static readonly byte[] iv2 = new byte[16] {
		0x67, 0x26, 0x35, 0xA7, 0x24, 0xA7, 0x37, 0x21, 0x73, 0x33, 0x6E, 0x6D, 0x34, 0x32, 0x64, 0x35
	};

	readonly IPEImage peImage;
	readonly List<UnpackedFile> satelliteAssemblies = new();
	string[] filenames;
	bool shouldUnpack;
	uint[] sizes;
	...
	byte[] Unpack2(MyPEImage peImage) {
		shouldUnpack = false;
		uint headerOffset = peImage.Length - 12;
		...
		byte[] ezencryptionLibData =
			Decompress1(peImage.OffsetReadBytes(ezencryptionLibOffset, (int)ezencryptionLibLength));
	...
	UnpackedFile UnpackEmbeddedFile(MyPEImage peImage, int index, ApplicationModeDecrypter decrypter) {
		...
		byte[] data = peImage.OffsetReadBytes(offset, (int)sizes[index + 1]);
	...
	static uint[] GetSizes(string sizes) {
		...
		foreach (string num in sizes.Split('|'))
