...
class NativeFileDecrypter {
	readonly byte kb;
	readonly byte[] key;
	readonly byte[,] transform = new byte[256, 256];

	public NativeFileDecrypter(byte[] keyData) {
		byte[] keyInit = new byte[] {
			0x78, 0x61, 0x32, keyData[0], keyData[2], 0x62, keyData[3], keyData[0], keyData[1], keyData[1], 0x66,
			keyData[1], keyData[5], 0x33, keyData[2], keyData[4], 0x74, 0x32, keyData[3], keyData[2]
		};
		key = new byte[32];
		for (int i = 0; i < 32; i++) {
		...
		ushort[,] transformTemp = new ushort[256, 256];
		for (int i = 0; i < 256; i++)
		for (int j = 0; j < 256; j++)
