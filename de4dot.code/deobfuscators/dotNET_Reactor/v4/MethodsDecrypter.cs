...
class MethodsDecrypter {
	...
	readonly EncryptedResource encryptedResource;
	readonly Dictionary<MethodDef, byte[]> methodToNativeMethod = new();
	readonly ModuleDefMD module;
	Dictionary<uint, byte[]> tokenToNativeMethod = new();
	...
	public bool Decrypt(MyPEImage peImage, ISimpleDeobfuscator simpleDeobfuscator, ref DumpedMethods dumpedMethods,
		...
		if (!encryptedResource.FoundResource)
			return false;
		byte[] methodsData = encryptedResource.Decrypt();
		...
		if ((tmp & 0xFF000000) == 0x06000000) {
			...
			while (methodsDataReader.Position < methodsData.Length - 1) {
				...
				byte[] methodData = methodsDataReader.ReadBytes(size);
				...
				var mbHeader = MethodBodyParser.ParseMethodBody(ref codeReader, out byte[] code, out dm.extraSections);
	...
	public void EncryptNativeMethods(ModuleWriterBase moduleWriter) {
		...
		foreach (var method in validNativeMethods) {
			byte[] code = methodToNativeMethod[method];
			...
			Logger.v("Native method {0:X8}, code RVA {1:X8}",
				new MDToken(Table.Method, moduleWriter.Metadata.GetRid(method)).Raw, codeRva);
