...
class ProxyCallFixer : ProxyCallFixer1 {
	readonly Dictionary<int, int> dictionary = new();
	...
	protected override object CheckCctor(ref TypeDef type, MethodDef cctor) {
		...
		if (instrs[instrs.Count - 3].OpCode != OpCodes.Ldtoken)
			return null;
		if (instrs[instrs.Count - 2].OpCode != OpCodes.Call ||
		    !IsDelegateCreatorMethod(instrs[instrs.Count - 2].Operand as MethodDef))
			return null;
	...
	protected override void GetCallInfo(object context, FieldDef field, out IMethod calledMethod,
		...
		if (!dictionary.TryGetValue(field.MDToken.ToInt32(), out int token))
	...
	void GetDictionary() {
		...
		byte[] resource = encryptedResource.Decrypt();
