...
public class DeobfuscatorInfo : DeobfuscatorInfoBase {
	...
	readonly BoolOption decryptBools;
	readonly BoolOption decryptMethods;
	readonly BoolOption decryptResources;
	readonly BoolOption dumpEmbeddedAssemblies;
	readonly BoolOption inlineMethods;
	readonly BoolOption removeAntiStrongName;
	readonly BoolOption removeInlinedMethods;
	readonly BoolOption removeNamespaces;
	readonly BoolOption renameShort;
	readonly BoolOption restoreTypes;
	...
	public DeobfuscatorInfo()
		...
		removeNamespaces = new BoolOption(null, MakeArgName("ns1"), "Clear namespace if there's only one class in it",
			true);
...
class Deobfuscator : DeobfuscatorBase {
	static readonly Regex isRandomName = new(@"^[A-Z]{30,40}$");

	static readonly Regex
		isRandomNameMembers = new(@"^(?:[a-zA-Z0-9]{9,11}|[a-zA-Z0-9]{18,20})$"); // methods, fields, props, events

	static readonly Regex isRandomNameTypes = new(@"^[a-zA-Z0-9]{18,20}(?:`\d+)?$"); // types, namespaces
	...
	public override byte[] UnpackNativeFile(IPEImage peImage) {
		byte[] data = new NativeImageUnpacker(peImage).Unpack();
		if (data == null)
			return null;
	...
	string DetectVersion() {
		...
		*/
		...
		if (!hasCorEnableProfilingString) {
			bool callsReverse = DotNetUtils.CallsMethod(methodsDecrypter.Method,
			...
			bool hasSymmetricAlgorithm =
				new LocalTypes(methodsDecrypter.Method).Exists("System.Security.Cryptography.SymmetricAlgorithm");
			if (module.IsClr40) {
	...
	static bool FindString(MethodDef method, string s) {
		foreach (string cs in DotNetUtils.GetCodeStrings(method)) {
			if (cs == s)
				return true;
		}
	...
	void RemoveMethods() {
		...
		foreach (var type in module.GetTypes()) {
			foreach (var method in type.Methods) {
				...
				foreach (string str in DotNetUtils.GetCodeStrings(method)) {
					if (str == " is tampered.") {
						antiTamper = method;
		...
		foreach (var type in module.GetTypes()) {
			foreach (var method in type.Methods) {
				...
				for (int i = 0; i < instr.Count; i++) {
					if (instr[i].OpCode == OpCodes.Call && instr[i].Operand is null && instr[i + 1].IsStloc() &&
					    instr[i + 2].IsLdloc()) {
	...
	void DumpEmbeddedAssemblies() {
		...
		foreach (var info in assemblyResolver.GetEmbeddedAssemblies(DeobfuscatedFile, this)) {
			string simpleName = Utils.GetAssemblySimpleName(info.name);
	...
	public override void DeobfuscateMethodBegin(Blocks blocks) => proxyCallFixer.Deobfuscate(blocks);
	...
	TypeDef GetDecrypterType() => methodsDecrypter.DecrypterType ??
	                              stringDecrypter.DecrypterType ?? booleanDecrypter.DecrypterType;
