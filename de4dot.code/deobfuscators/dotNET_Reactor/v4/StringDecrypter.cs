...
class StringDecrypter {
	readonly List<DecrypterInfo> decrypterInfos = new();
	...
	public MethodDef OtherStringDecrypter { get; private set; }

	T Lookup<T>(T def, string errorMessage) where T : class, ICodedToken =>
		DeobUtils.Lookup(module, def, errorMessage);

	public void Find(ISimpleDeobfuscator simpleDeobfuscator) {
		string[] additionalTypes = new[] { "System.String" };
	...
	void FindKeyIv(MethodDef method, out byte[] key, out byte[] iv) {
		...
		string[] requiredTypes = new[] {
			"System.Byte[]", "System.IO.MemoryStream", "System.Security.Cryptography.CryptoStream"
		};
