	...
	public bool Deobfuscate(List<Block> allBlocks) {
		...
		foreach (var block in allBlocks) {
			...
			if (!lastInstr.IsBrtrue() && !lastInstr.IsBrfalse())
				continue;
			int callIndex = instrs.IndexOf(block.LastInstr) - 1;
			...
			bool flag = method.ReturnType.FullName == typeof(bool).FullName &&
			            methodInstrs[methodInstrs.Count - 2].OpCode.Code != Code.Ldc_I4_0;
