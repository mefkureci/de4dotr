...
class EncryptedResource {
	static readonly int[] pktIndexes = new int[16] { 1, 0, 3, 1, 5, 2, 7, 3, 9, 4, 11, 5, 13, 6, 15, 7 };
	...
	public EncryptedResource(ModuleDefMD module, EncryptedResource oldOne) {
		this.module = module;
		Method = Lookup(oldOne.Method, "Could not find resource decrypter method");
		if (oldOne.Resource != null)
			Resource = DotNetUtils.GetResource(module, oldOne.Resource.Name.String) as EmbeddedResource;
		decrypter = oldOne.decrypter;

		if (Resource == null && oldOne.Resource != null)
			throw new ApplicationException("Could not initialize EncryptedResource");
	}

	public DnrDecrypterType DecrypterTypeVersion =>
		decrypter == null ? DnrDecrypterType.Unknown : decrypter.DecrypterType;
	...
	public bool FoundResource => Resource != null;

	public void SetNewResource(byte[] data) {
		var dataReaderFactory = ByteArrayDataReaderFactory.Create(data, null);
		var newResource =
			new EmbeddedResource(Resource.Name, dataReaderFactory, 0, (uint)data.Length, Resource.Attributes);
		int index = module.Resources.IndexOf(Resource);
	...
	public DnrDecrypterType GuessDecrypterType() => GetDecrypterType(Method, null);
	...
	public void Initialize(ISimpleDeobfuscator simpleDeobfuscator) {
		...
		Resource = FindMethodsDecrypterResource(Method);
		if (Resource == null)
			return;

		var decrypterType = GetDecrypterType(Method, new string[0]);

		if (decrypterType == DnrDecrypterType.V3) {
			decrypter = new DecrypterV3(Method);
		}
		else if (decrypterType == DnrDecrypterType.V4) {
			decrypter = new DecrypterV4(module, simpleDeobfuscator, Method);
		}
		else {
			byte[] key = ArrayFinder.GetInitializedByteArray(Method, 32);
			if (key == null)
				throw new ApplicationException("Could not find resource decrypter key");
			byte[] iv = ArrayFinder.GetInitializedByteArray(Method, 16);
			if (iv == null)
			...
			case DnrDecrypterType.V2: decrypter = new DecrypterV2(iv, key, Method); break;
	...
	bool NeedReverse() => DotNetUtils.CallsMethodContains(Method, "System.Array::Reverse");
	...
	public byte[] Decrypt() {
		if (Resource == null || decrypter == null)
			throw new ApplicationException("Can't decrypt resource");
		return decrypter.Decrypt(Resource);
	}
	...
	class DecrypterV1 : IDecrypter {
		...
		public byte[] Decrypt(EmbeddedResource resource) =>
			DeobUtils.AesDecrypt(resource.CreateReader().ToArray(), key, iv);
		...
		public static bool CouldBeResourceDecrypter(MethodDef method, LocalTypes localTypes,
			...
			if (DotNetUtils.GetMethod(method.DeclaringType, "System.Security.Cryptography.SymmetricAlgorithm", "()") !=
			    null) {
	...
	class DecrypterV2 : IDecrypter {
		readonly InstructionEmulator instrEmulator = new();
		readonly byte[] key, iv;
		...
		bool Initialize() {
			...
			if (!Find(origInstrs, out int emuStartIndex, out int emuEndIndex, out emuLocal)) {
				if (!FindStartEnd(origInstrs, out emuStartIndex, out emuEndIndex, out emuLocal))
					if (!FindStartEnd2(ref origInstrs, out emuStartIndex, out emuEndIndex, out emuLocal, out emuArg,
						    ref emuMethod, ref locals))
						return false;
					else
						isNewDecrypter = true;
			}
		...
		bool FindStartEnd2(ref IList<Instruction> instrs, out int startIndex, out int endIndex, out Local tmpLocal,
			out Parameter tmpArg, ref MethodDef methodDef, ref List<Local> locals) {
	...
	class DecrypterV3 : IDecrypter {
		...
		public static bool CouldBeResourceDecrypter(MethodDef method, LocalTypes localTypes,
			IList<string> additionalTypes) {
			var requiredTypes = new List<string> {
				"System.Reflection.Emit.DynamicMethod", "System.Reflection.Emit.ILGenerator"
			};
			requiredTypes.AddRange(additionalTypes);
			if (!localTypes.All(requiredTypes))
				return false;
