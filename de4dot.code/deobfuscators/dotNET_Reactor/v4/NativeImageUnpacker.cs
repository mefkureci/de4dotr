...
class NativeImageUnpacker {
	...
	public byte[] Unpack() {
		...
		byte[] encryptedData = dataEntry.CreateReader().ToArray();
	...
	static byte[] UnpackLoader(byte[] loaderData) {
		byte[] loaderBytes = new byte[loaderData.Length - loaderHeaderSizeV45];
	...
	byte[] GetKeyData() {
		...
		for (int i = 0; i < baseOffsets.Length; i++) {
			byte[] code = peImage.OffsetReadBytes(baseOffsets[i], decryptMethodPattern.Length);
			if (DeobUtils.IsCode(decryptMethodPattern, code))
				return GetKeyData(baseOffsets[i]);
		}

		byte[] net1xCode = peImage.OffsetReadBytes(0x207E0, startMethodNet1xPattern.Length);
	...
	byte[] GetKeyData(uint baseOffset) =>
		new byte[6] {
			peImage.OffsetReadByte(baseOffset + 5), peImage.OffsetReadByte(baseOffset + 0xF),
			peImage.OffsetReadByte(baseOffset + 0x58), peImage.OffsetReadByte(baseOffset + 0x6D),
			peImage.OffsetReadByte(baseOffset + 0x98), peImage.OffsetReadByte(baseOffset + 0xA6)
		};
}
