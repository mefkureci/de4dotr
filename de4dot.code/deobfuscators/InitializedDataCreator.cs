...
public class InitializedDataCreator {
	...
	readonly Dictionary<long, TypeDef> sizeToArrayType = new();
	...
	MemberRef CreateInitializeArrayMethod() {
		if (initializeArrayMethod == null) {
			var runtimeHelpersType = DotNetUtils.FindOrCreateTypeRef(module, module.CorLibTypes.AssemblyRef,
				"System.Runtime.CompilerServices", "RuntimeHelpers", false);
			var systemArrayType =
				DotNetUtils.FindOrCreateTypeRef(module, module.CorLibTypes.AssemblyRef, "System", "Array", false);
			var runtimeFieldHandleType = DotNetUtils.FindOrCreateTypeRef(module, module.CorLibTypes.AssemblyRef,
			...
			initializeArrayMethod = module.UpdateRowId(new MemberRefUser(module, "InitializeArray", methodSig,
	...
	void CreateOurType() {
		...
		ourType = new TypeDefUser("", $"<PrivateImplementationDetails>{GetModuleId()}",
	...
	object GetModuleId() {
		...
		byte[] hash = new SHA1Managed().ComputeHash(memoryStream.GetBuffer());
	...
	TypeDef GetArrayType(long size) {
		...
		if (valueType == null)
			valueType = DotNetUtils.FindOrCreateTypeRef(module, module.CorLibTypes.AssemblyRef, "System", "ValueType",
				false);
