...
public abstract class DeobfuscatorBase : IDeobfuscator, IModuleWriterListener {
	...
	readonly IList<RemoveInfo<TypeDef>> attrsToRemove = new List<RemoveInfo<TypeDef>>();
	readonly IList<RemoveInfo<FieldDef>> fieldsToRemove = new List<RemoveInfo<FieldDef>>();
	readonly MethodCallRemover methodCallRemover = new();
	readonly IList<RemoveInfo<MethodDef>> methodsToRemove = new List<RemoveInfo<MethodDef>>();
	readonly List<string> namesToPossiblyRemove = new();
	readonly Dictionary<object, bool> objectsThatMustBeKept = new();

	readonly OptionsBase optionsBase;
	readonly IList<RemoveInfo<Resource>> resourcesToRemove = new List<RemoveInfo<Resource>>();
	readonly IList<RemoveInfo<TypeDef>> typesToRemove = new List<RemoveInfo<TypeDef>>();
	protected InitializedDataCreator initializedDataCreator;
	MetadataFlags? mdFlags;
	protected ModuleDefMD module;
	protected StaticStringInliner staticStringInliner = new();
	...
	protected byte[] ModuleBytes { get; set; }
	...
	protected bool CanRemoveStringDecrypterType =>
		Operations.DecryptStrings != OpDecryptString.None && staticStringInliner.InlinedAllCalls;
	...
	void DeleteOtherAttributes(IList<CustomAttribute> customAttributes) {
		for (int i = customAttributes.Count - 1; i >= 0; i--) {
			string attr = customAttributes[i].TypeFullName;
	...
	protected void AddResources(string reason) {
		...
		foreach (string name in namesToPossiblyRemove) {
	...
	protected void RemoveInlinedMethods(List<MethodDef> inlinedMethods) =>
		AddMethodsToBeRemoved(new UnusedMethodsFinder(module, inlinedMethods, GetRemovedMethods()).Find(),
			"Inlined method");
	...
	class MethodCallRemover {
		readonly Dictionary<string, MethodDefAndDeclaringTypeDict<bool>> methodNameInfos = new();
		readonly MethodDefAndDeclaringTypeDict<MethodDefAndDeclaringTypeDict<bool>> methodRefInfos = new();
