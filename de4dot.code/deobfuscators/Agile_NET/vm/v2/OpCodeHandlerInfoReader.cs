...
class OpCodeHandlerInfoReader {
	static readonly List<ConvInfo> instructionInfos1 = new() {
	...
	readonly Dictionary<HandlerTypeCode, Func<BinaryReader, Instruction>> readHandlers;
	readonly IInstructionOperandResolver resolver;
	...
	Instruction Handler_Beq(BinaryReader reader) => new(OpCodes.Beq, new TargetDisplOperand(reader.ReadInt32()));
	Instruction Handler_Bge(BinaryReader reader) => new(OpCodes.Bge, new TargetDisplOperand(reader.ReadInt32()));
	Instruction Handler_Bge_Un(BinaryReader reader) => new(OpCodes.Bge_Un, new TargetDisplOperand(reader.ReadInt32()));
	Instruction Handler_Bgt(BinaryReader reader) => new(OpCodes.Bgt, new TargetDisplOperand(reader.ReadInt32()));
	Instruction Handler_Bgt_Un(BinaryReader reader) => new(OpCodes.Bgt_Un, new TargetDisplOperand(reader.ReadInt32()));
	Instruction Handler_Ble(BinaryReader reader) => new(OpCodes.Ble, new TargetDisplOperand(reader.ReadInt32()));
	Instruction Handler_Ble_Un(BinaryReader reader) => new(OpCodes.Ble_Un, new TargetDisplOperand(reader.ReadInt32()));
	Instruction Handler_Blt(BinaryReader reader) => new(OpCodes.Blt, new TargetDisplOperand(reader.ReadInt32()));
	Instruction Handler_Blt_Un(BinaryReader reader) => new(OpCodes.Blt_Un, new TargetDisplOperand(reader.ReadInt32()));
	Instruction Handler_Bne_Un(BinaryReader reader) => new(OpCodes.Bne_Un, new TargetDisplOperand(reader.ReadInt32()));

	Instruction Handler_Box(BinaryReader reader) =>
		OpCodes.Box.ToInstruction(resolver.ResolveToken(reader.ReadUInt32(), gpContext) as ITypeDefOrRef);

	Instruction Handler_Br(BinaryReader reader) => new(OpCodes.Br, new TargetDisplOperand(reader.ReadInt32()));

	Instruction Handler_Brfalse(BinaryReader reader) =>
		new(OpCodes.Brfalse, new TargetDisplOperand(reader.ReadInt32()));

	Instruction Handler_Brtrue(BinaryReader reader) => new(OpCodes.Brtrue, new TargetDisplOperand(reader.ReadInt32()));

	Instruction Handler_Call(BinaryReader reader) =>
		OpCodes.Call.ToInstruction(resolver.ResolveToken(reader.ReadUInt32(), gpContext) as IMethod);

	Instruction Handler_Callvirt(BinaryReader reader) =>
		OpCodes.Callvirt.ToInstruction(resolver.ResolveToken(reader.ReadUInt32(), gpContext) as IMethod);

	Instruction Handler_Castclass(BinaryReader reader) =>
		OpCodes.Castclass.ToInstruction(resolver.ResolveToken(reader.ReadUInt32(), gpContext) as ITypeDefOrRef);
	...
	Instruction Handler_Initobj(BinaryReader reader) =>
		OpCodes.Initobj.ToInstruction(resolver.ResolveToken(reader.ReadUInt32(), gpContext) as ITypeDefOrRef);

	Instruction Handler_Isinst(BinaryReader reader) =>
		OpCodes.Isinst.ToInstruction(resolver.ResolveToken(reader.ReadUInt32(), gpContext) as ITypeDefOrRef);

	Instruction Handler_Ldarg(BinaryReader reader) => new(OpCodes.Ldarg, new ArgOperand(reader.ReadUInt16()));
	Instruction Handler_Ldarga(BinaryReader reader) => new(OpCodes.Ldarga, new ArgOperand(reader.ReadUInt16()));
	...
	Instruction Handler_Ldelem(BinaryReader reader) => new(OpCodes.Ldelem, null);
	Instruction Handler_Ldelema(BinaryReader reader) => new(OpCodes.Ldelema, null);
	...
	Instruction Handler_Ldftn(BinaryReader reader) =>
		OpCodes.Ldftn.ToInstruction(resolver.ResolveToken(reader.ReadUInt32(), gpContext) as IMethod);

	Instruction Handler_Ldlen(BinaryReader reader) => OpCodes.Ldlen.ToInstruction();
	Instruction Handler_Ldloc(BinaryReader reader) => new(OpCodes.Ldloc, new LocalOperand(reader.ReadUInt16()));
	Instruction Handler_Ldloca(BinaryReader reader) => new(OpCodes.Ldloca, new LocalOperand(reader.ReadUInt16()));
	Instruction Handler_Ldobj(BinaryReader reader) => new(OpCodes.Ldobj, null);
	...
	Instruction Handler_Ldtoken(BinaryReader reader) =>
		OpCodes.Ldtoken.ToInstruction(resolver.ResolveToken(reader.ReadUInt32(), gpContext) as ITokenOperand);
	...
	Instruction Handler_Leave(BinaryReader reader) => new(OpCodes.Leave, new TargetDisplOperand(reader.ReadInt32()));
	...
	Instruction Handler_Newarr(BinaryReader reader) =>
		OpCodes.Newarr.ToInstruction(resolver.ResolveToken(reader.ReadUInt32(), gpContext) as ITypeDefOrRef);

	Instruction Handler_Newobj(BinaryReader reader) =>
		OpCodes.Newobj.ToInstruction(resolver.ResolveToken(reader.ReadUInt32(), gpContext) as IMethod);
	...
	Instruction Handler_Starg(BinaryReader reader) => new(OpCodes.Starg, new ArgOperand(reader.ReadUInt16()));
	Instruction Handler_Stelem(BinaryReader reader) => new(OpCodes.Stelem, null);
	...
	Instruction Handler_Stobj(BinaryReader reader) => new(OpCodes.Stobj, null);
	...
	Instruction Handler_Unbox_Any(BinaryReader reader) =>
		OpCodes.Unbox_Any.ToInstruction(resolver.ResolveToken(reader.ReadUInt32(), gpContext) as ITypeDefOrRef);
