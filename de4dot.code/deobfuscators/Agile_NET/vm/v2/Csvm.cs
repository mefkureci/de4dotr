...
class Csvm {
	...
	AssemblyRef FindVmAssemblyRef() {
		foreach (var memberRef in module.GetMemberRefs()) {
			...
			if (memberRef.Name != "RunMethod")
				continue;
			if (memberRef.FullName ==
			    "System.Object VMRuntime.Libraries.CSVMRuntime::RunMethod(System.String,System.Object[])")
	...
	VmOpCodeHandlerDetector GetVmOpCodeHandlerDetector() {
		...
		string dataKey = "cs cached VmOpCodeHandlerDetector v2";
		var dict = (Dictionary<string, VmOpCodeHandlerDetector>)deobfuscatorContext.GetData(dataKey);
		if (dict == null)
			deobfuscatorContext.SetData(dataKey,
				dict = new Dictionary<string, VmOpCodeHandlerDetector>(StringComparer.OrdinalIgnoreCase));
