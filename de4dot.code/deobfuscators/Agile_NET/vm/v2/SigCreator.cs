...
class BlockSigInfo {
	...
	public List<BlockElementHash> Hashes { get; }
...
enum BlockElementHash {
}

class SigCreator {
	const int BASE_INDEX = 0x40000000;
	readonly Dictionary<object, int> objToId = new();
	Blocks blocks;
	CRC32 hasher;
	...
	public List<BlockSigInfo> Create(MethodDef method) {
		...
		foreach (var block in allBlocks) {
			var blockInfo = new BlockSigInfo {
				HasFallThrough = block.FallThrough != null, EndsInRet = block.LastInstr.OpCode.Code == Code.Ret
			};
