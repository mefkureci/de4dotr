...
static class OpCodeHandlerInfos {
	public static readonly IList<MethodSigInfo>[] HandlerInfos = new[] {
		ReadOpCodeHandlerInfos(CsvmResources.CSVM1), ReadOpCodeHandlerInfos(CsvmResources.CSVM2),
		ReadOpCodeHandlerInfos(CsvmResources.CSVM3), ReadOpCodeHandlerInfos(CsvmResources.CSVM4),
		ReadOpCodeHandlerInfos(CsvmResources.CSVM5), ReadOpCodeHandlerInfos(CsvmResources.CSVM6)
	};
		...
		for (int i = 0; i < numHandlers; i++) {
			...
			for (int j = 0; j < numBlocks; j++) {
				...
				var block = new BlockSigInfo(hashes, targets) {
					HasFallThrough = reader.ReadBoolean(), EndsInRet = reader.ReadBoolean()
				};
				blocks.Add(block);
			}
