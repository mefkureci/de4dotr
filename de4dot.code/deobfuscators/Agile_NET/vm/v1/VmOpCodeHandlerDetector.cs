...
class VmOpCodeHandlerDetector {
	...
	public List<OpCodeHandler> Handlers { get; private set; }
	...
	List<TypeDef> FindVmHandlerTypes() {
		string[] requiredFields = new[] {
			null, "System.Collections.Generic.Dictionary`2<System.UInt16,System.Type>", "System.UInt16"
		};
	...
	void DetectHandlers(List<TypeDef> handlerTypes, CsvmInfo csvmInfo) {
		Handlers = new List<OpCodeHandler>();
		...
		foreach (var handlersList in OpCodeHandlers.Handlers) {
			...
			foreach (var handlerType in handlerTypes) {
				...
				if (detected.Count != 1)
					goto next;
				Handlers.Add(detected[0]);
			}

			if (new List<OpCodeHandler>(Utils.Unique(Handlers)).Count == Handlers.Count)
				return;
			next: ;
		}
