...
static partial class OpCodeHandlers {
	static readonly List<InstructionInfo1> instructionInfos1 = new() {
	...
	static readonly List<InstructionInfo2> instructionInfos2 = new() {
	...
	static Instruction newarr_read(BinaryReader reader, IInstructionOperandResolver resolver,
		GenericParamContext gpContext) =>
		new(OpCodes.Newarr, resolver.ResolveToken(reader.ReadUInt32(), gpContext));
	...
	static bool endfinally_check(UnknownHandlerInfo info) =>
		DotNetUtils.CallsMethod(info.ExecuteMethod,
	...
	static Instruction initobj_read(BinaryReader reader, IInstructionOperandResolver resolver,
		GenericParamContext gpContext) =>
		new(OpCodes.Initobj, resolver.ResolveToken(reader.ReadUInt32(), gpContext));
	...
	static Instruction ldtoken_read(BinaryReader reader, IInstructionOperandResolver resolver,
		GenericParamContext gpContext) =>
		new(OpCodes.Ldtoken, resolver.ResolveToken(reader.ReadUInt32(), gpContext));
	...
	static bool nop_check(UnknownHandlerInfo info) =>
		IsEmptyMethod(info.ReadMethod) && IsEmptyMethod(info.ExecuteMethod);
