...
abstract class CsvmToCilMethodConverterBase {
	readonly Dictionary<Instruction, int> cilToVmIndex = new();
	readonly IDeobfuscatorContext deobfuscatorContext;
	protected readonly ModuleDefMD module;
	readonly CilOperandInstructionRestorer operandRestorer = new();
	readonly Dictionary<int, Instruction> vmIndexToCil = new();
	...
	public void Convert(MethodDef cilMethod, CsvmMethodData csvmMethod) {
		...
		if (!operandRestorer.Restore(cilMethod))
			Logger.w("Failed to restore one or more instruction operands in CSVM method {0:X8}",
				cilMethod.MDToken.ToInt32());
	...
	object FixOperand(IList<Instruction> instrs, Instruction instr, IVmOperand vmOperand) {
		...
		if (vmOperand is SwitchTargetDisplOperand) {
			int[] targetDispls = ((SwitchTargetDisplOperand)vmOperand).TargetDisplacements;
		...
		if (vmOperand is FieldInstructionOperand fieldInfo)
			return FixLoadStoreFieldInstruction(instr, fieldInfo.Field, fieldInfo.StaticOpCode,
				fieldInfo.InstanceOpCode);
	...
	IField FixLoadStoreFieldInstruction(Instruction instr, IField fieldRef, OpCode staticInstr, OpCode instanceInstr) {
		...
		bool isStatic;
		if (field == null) {
			Logger.w("Could not resolve field {0:X8}. Assuming it's not static.",
				fieldRef == null ? 0 : fieldRef.MDToken.Raw);
			isStatic = false;
		}
		else
