...
using de4dot.blocks;
using de4dot.code.deobfuscators.Agile_NET.vm.v1;
using dnlib.DotNet;
using dnlib.PE;
...
public class DeobfuscatorInfo : DeobfuscatorInfoBase {
	...
	readonly BoolOption removeStackFrameHelper;
	readonly BoolOption restoreVmCode;
	readonly BoolOption setInitLocals;
...
class Deobfuscator : DeobfuscatorBase {
	...
	List<TypeDef> cliSecureAttributes = new();
	...
	public override byte[] UnpackNativeFile(IPEImage peImage) =>
		UnpackNativeFile1(peImage) ?? UnpackNativeFile2(peImage);
	...
	byte[] UnpackNativeFile1(IPEImage peImage) {
		...
		byte[] fileData = peImage.CreateReader().ToArray();
	...
	protected override void ScanForObfuscator() {
		...
		csvmV1 = new Csvm(DeobfuscatedFile.DeobfuscatorContext, module);
		csvmV1.Find();
	...
	void FindCliSecureAttribute() {
		...
		foreach (var type in module.Types) {
			if (Utils.StartsWith(type.FullName, "SecureTeam.Attributes.ObfuscatedByCliSecureAttribute",
				    StringComparison.Ordinal)) {
				cliSecureAttributes.Add(type);
				obfuscatorName = "CliSecure";
			}
			else if (Utils.StartsWith(type.FullName, "SecureTeam.Attributes.ObfuscatedByAgileDotNetAttribute",
				         StringComparison.Ordinal)) {
				cliSecureAttributes.Add(type);
	...
	public override IDeobfuscator ModuleReloaded(ModuleDefMD module) {
		...
		newOne.csvmV1 = new Csvm(DeobfuscatedFile.DeobfuscatorContext, module, csvmV1);
