...
class MethodsDecrypter {
	...
	List<CsHeaderVersion> GetCsHeaderVersions(uint codeHeaderOffset, MDTable methodDefTable) {
		...
		if (IsCsHeader40(codeHeaderOffset)) {
			return new List<CsHeaderVersion> { CsHeaderVersion.V40, CsHeaderVersion.V30 };
		}

		return new List<CsHeaderVersion> { CsHeaderVersion.V45, CsHeaderVersion.V50 };
	}
	...
	bool IsOldHeader(MDTable methodDefTable) {
		if (methodDefTable.RowSize != codeHeader.methodDefElemSize)
			return true;
		if ((uint)methodDefTable.StartOffset - peImage.RvaToOffset((uint)peImage.Cor20Header.Metadata.VirtualAddress) !=
	...
	public bool Decrypt(MyPEImage peImage, ModuleDefMD module, CliSecureRtType csRtType,
		...
		case DecryptResult.Error:
			...
			dumpedMethods = deobfuscators.MethodsDecrypter.Decrypt(module, moduleCctorBytes);
			return true;
	...
	static byte[] GetModuleCctorBytes(CliSecureRtType csRtType) {
		...
		byte[] moduleCctorBytes = new byte[6];
	...
	void DecryptMethods(uint codeHeaderOffset, MDTable methodDefTable, ICsHeader csHeader,
		ref DumpedMethods dumpedMethods) {
	...
	static SigType GetSigType(byte[] sig) {
		...
		if (Utils.Compare(sig, proSignature))
			return SigType.Pro;
		if (Utils.Compare(sig, oldSignature))
			return SigType.Old;
	...
	class CsHeader30 : CsHeaderBase {
		...
		public override IDecrypter CreateDecrypter() =>
			new Decrypter30(methodsDecrypter.peImage, methodsDecrypter.codeHeader);
	...
	class CsHeader40 : CsHeaderBase {
		...
		public override IDecrypter CreateDecrypter() =>
			new Decrypter40(methodsDecrypter.peImage, methodsDecrypter.codeHeader);
	...
	class CsHeader45 : CsHeaderBase {
		...
		public override IDecrypter CreateDecrypter() =>
			new Decrypter45(methodsDecrypter.peImage, methodsDecrypter.codeHeader);
