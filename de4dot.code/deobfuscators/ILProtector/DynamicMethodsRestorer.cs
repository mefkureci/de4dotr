...
class DynamicMethodsRestorer : MethodsDecrypterBase {
	...
	protected override void DecryptInternal() {
		...
		using (var client = new NewProcessAssemblyClientFactory(serverVersion).Create(AssemblyServiceType.Generic)) {
			...
			decryptedData =
				client.GenericService.SendMessage(DynamicMethodsDecrypterService.MSG_DECRYPT_METHODS,
					new object[] { GetMethodIds() }) as IList<DecryptedMethodInfo>;
			MethodReaderHasDelegateTypeFlag =
				(bool)client.GenericService.SendMessage(DynamicMethodsDecrypterService.MSG_HAS_DELEGATE_TYPE_FLAG,
					new object[0]);
		}
