...
class StaticMethodsDecrypter : MethodsDecrypterBase {
	...
	class DecrypterBase : IDecrypter {
		protected static readonly byte[] ilpPublicKeyToken =
			new byte[8] { 0x20, 0x12, 0xD3, 0xC0, 0x55, 0x1F, 0xE0, 0x3D };
	...
	class DecrypterV100 : DecrypterBase {
		...
		public static DecrypterV100 Create(ref DataReader reader) {
			...
			return new DecrypterV100(new Version(reader.ReadByte(), reader.ReadByte(), reader.ReadByte(),
				reader.ReadByte()));
		}
	}
	...
	class DecrypterV105 : DecrypterBase {
		...
		public static DecrypterV105 Create(ref DataReader reader) {
			...
			if (reader.Length < 0xA4)
				return null;
			byte[] key = reader.ReadBytes(0x94);
			if (!Utils.Compare(reader.ReadBytes(8), ilpPublicKeyToken))
				return null;
			return new DecrypterV105(
				new Version(reader.ReadByte(), reader.ReadByte(), reader.ReadByte(), reader.ReadByte()), key);
		}
	}

	// 1.0.6.x
	class DecrypterV106 : DecrypterBase {
		readonly byte[] decryptionKey6;
		readonly byte[] decryptionKey7;
		...
		public static DecrypterV106 Create(ref DataReader reader) {
			try {
				...
				reader.Position =
					(uint)(keyXorOffs7 + (ReadByteAt(ref reader, 1) ^ ReadByteAt(ref reader, keyXorOffs7)));
				...
				byte[] sha1Data = reader.ReadBytes(sha1DataLen);
				...
				byte[] key0 = DeobUtils.Sha1Sum(sha1Data); // 1.0.6.0
				byte[] key6 = GetKey(ref reader, key0, keyXorOffs6); // 1.0.6.6
				byte[] key7 = GetKey(ref reader, key0, keyXorOffs7); // 1.0.6.7
		...
		public override byte[] GetMethodsData(EmbeddedResource resource) {
			var reader = resource.CreateReader();
			reader.Position = (uint)startOffset;
			byte[] decrypted = new byte[reader.Read7BitEncodedUInt32()];
			uint origCrc32 = reader.ReadUInt32();
			uint pos = reader.Position;

			byte[][] keys = new[] { decryptionKey, decryptionKey6, decryptionKey7 };
