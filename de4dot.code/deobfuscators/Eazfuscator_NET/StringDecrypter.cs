...
class StringDecrypter : IDisposable {
	static readonly string[] requiredFieldTypes = new[] { "System.Byte[]", "System.Int16" };

	static readonly string[] streamHelperTypeFields = new[] { "System.IO.Stream", "System.Byte[]" };

	static readonly string[] requiredLocalTypes = new[] {
		"System.Boolean", "System.Byte[]", "System.Char[]", "System.Int16", "System.Int32",
		"System.Reflection.Assembly", "System.String"
	};

	readonly DecrypterType decrypterType;
	...
	public int? ValidStringDecrypterValue { get; private set; }

	public TypeDef Type { get; private set; }

	public EmbeddedResource Resource { get; private set; }

	public IEnumerable<TypeDef> Types => new List<TypeDef> { Type, dataDecrypterType };
	...
	public MethodDef RealMethod => realMethod ?? Method;
	...
	public void Find() {
		foreach (var type in module.Types) {
			...
			foreach (var method in type.Methods) {
				...
				Type = type;
				isV32OrLater = CheckIfV32OrLater(Type);
				return;
			}
		}
	}
	...
	bool CheckType(TypeDef type) {
		...
		if (type.NestedTypes.Count == 0) {
			return DotNetUtils.FindFieldType(type, "System.IO.BinaryReader", true) != null &&
			       DotNetUtils.FindFieldType(type,
				       "System.Collections.Generic.Dictionary`2<System.Int32,System.String>", true) != null;
		}
	...
	static bool CheckDecrypterMethod(MethodDef method) {
		...
		if (!(DotNetUtils.IsMethod(method, "System.String", "(System.Int32)") ||
		      DotNetUtils.IsMethod(method, "System.String", "(System.Int32,System.Boolean)")))
			return false;
		...
		foreach (var instr in method.Body.Instructions) {
			if (instr.OpCode != OpCodes.Callvirt)
				continue;
			if (instr.Operand is IMethod calledMethod && calledMethod.FullName ==
	...
	bool FindConstants(ISimpleDeobfuscator simpleDeobfuscator) {
		...
		stringMethodConsts = new EfConstantsReader(Method);

		if (!FindResource(Method))
			return false;

		checkMinus2 = isV32OrLater || DeobUtils.HasInteger(Method, -2);
		usePublicKeyToken = CallsGetPublicKeyToken(Method);

		var int64Method = FindInt64Method(Method);
		if (int64Method != null)
		...
		if (!isV50OrLater) {
			dataDecrypterType = FindDataDecrypterType(Method);
			if (dataDecrypterType == null)
				return false;
		}

		if (isV32OrLater) {
			int index = FindInitIntsIndex(Method, out bool initializedAll);
			...
			if (!isV50OrLater)
				decrypterType.ShiftConsts = new List<int> {
					24,
					16,
					8,
					0,
	...
	List<FlagsInfo> FindFlags2(int index) {
		...
		for (int i = index - 1; i >= 0; i--) {
			...
			if ((uint)value != 0x80000000 && value != 0x40000000 && value != 0x20000000)
				continue;
			var local = GetFlagsLocal(Method, index2);
			if (local == null)
				continue;
			int offset = GetFlagsOffset(Method, index2, local);
			if (offset < 0)
				continue;
	...
	void Initialize() {
		reader = new BinaryReader(Resource.CreateReader().AsStream());
	...
	public string Decrypt(int val) {
		...
		while (true) {
			...
			if (checkMinus2 && flags == -2) {
				byte[] ary2 = reader.ReadBytes(4);
				val = -(magic1 ^ i5) ^ (ary2[2] | (ary2[0] << 8) | (ary2[3] << 16) | (ary2[1] << 24));
				continue;
			}

			byte[] bytes = reader.ReadBytes(flags & 0x1FFFFFFF);
	...
	static byte[] rld(byte[] src) {
		byte[] dst = new byte[src[2] + (src[3] << 8) + (src[0] << 16) + (src[1] << 24)];
		int srcIndex = 4;
		int dstIndex = 0;
		int flags = 0;
		int bit = 128;
	...
	bool FindResource(MethodDef method) {
		Resource = FindResourceFromCodeString(method) ??
	...
	EmbeddedResource FindResourceFromStringBuilder(MethodDef method) {
		int startIndex =
			EfUtils.FindOpCodeIndex(method, 0, Code.Newobj, "System.Void System.Text.StringBuilder::.ctor()");
		if (startIndex < 0)
			return null;
		int endIndex = EfUtils.FindOpCodeIndex(method, startIndex, Code.Call,
		...
		for (int i = startIndex; i < endIndex; i++) {
			var instr = instrs[i];
			if (instr.OpCode.Code == Code.Call && instr.Operand.ToString() ==
	...
	bool FindInts(int index) {
		...
		for (int i = index; i < instrs.Count - 2; i++) {
			...
			case Code.Ldsfld:
				...
				if (field == null || field.DeclaringType != Method.DeclaringType ||
				    field.FieldType.GetElementType() != ElementType.I4)
					goto default;
				fields[field] = null;
		...
		foreach (int? val in fields.Values) {
			if (val == null)
				continue;
			magic1 = /*i2 =*/ val.Value;
			return true;
	...
	bool EmulateDynocodeOld(InstructionEmulator emu, ref int index) {
		...
		if (ctor == null || ctor.MethodSig.GetParamCount() != 1 ||
		    ctor.MethodSig.Params[0].ElementType != ElementType.I4)
			return false;
		...
		var initValue = emu.GetLocal(ldloc.GetLocal(Method.Body.Variables)) as Int32Value;
	...
	static int GetIndexOfCall(IList<Instruction> instrs, int startIndex, int endIndex, string returnType,
		string parameters) {
	...
	class StreamHelperType {
		public readonly MethodDef readBytesMethod;
		public readonly MethodDef readInt16Method;
		public readonly MethodDef readInt32Method;
		public TypeDef type;
