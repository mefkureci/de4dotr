...
public abstract class TypesRestorerBase {
	readonly Dictionary<Parameter, TypeInfo<Parameter>> argInfos = new();
	readonly Dictionary<IField, TypeInfo<FieldDef>> fieldWrites = new(FieldEqualityComparer.CompareDeclaringTypes);
	readonly ModuleDef module;
	readonly Dictionary<int, UpdatedField> updatedFields = new();
	readonly Dictionary<int, UpdatedMethod> updatedMethods = new();
	...
	void RestoreFieldTypes() {
		...
		foreach (var updatedField in fields)
			Logger.v("Field {0:X8}: type {1} ({2:X8})", updatedField.token,
				Utils.RemoveNewlines(updatedField.newFieldType.FullName), updatedField.newFieldType.MDToken.ToInt32());
			...
			for (int i = 0; i < instructions.Count; i++) {
				...
				case Code.Newobj:
					...
					calledMethodArgs = DotNetUtils.ReplaceGenericParameters(
						calledMethodDefOrRef.DeclaringType.TryGetGenericInstSig(), calledMethodSpec, calledMethodArgs);
					for (int j = 0; j < pushedArgs.NumValidArgs; j++) {
	...
	TypeSig GetLoadedType(IGenericParameterProvider gpp, MethodDef method, IList<Instruction> instructions,
