...
static class Lzmat {
	...
	static uint LZMAT_GET_U4(byte[] _p_, ref uint _i_, ref byte _n_) =>
		(_n_ ^= 1) != 0 ? (uint)(_p_[_i_] & 0xF) : (uint)(_p_[_i_++] >> 4);

	static byte LZMAT_GET_U8(byte[] _p_, uint _i_, byte _n_) =>
		(byte)(_n_ != 0 ? (_p_[_i_] >> 4) | (_p_[_i_ + 1] << 4) : _p_[_i_]);

	static ushort LZMAT_GET_LE16(byte[] _p_, uint _i_, byte _n_) =>
		(ushort)(_n_ != 0 ? (_p_[_i_] >> 4) | (GET_LE16(_p_, _i_ + 1) << 4) : GET_LE16(_p_, _i_));
				...
				if ((tag & 0x80) != 0) // gamma
				{
					...
					if (outPos > MAX_LZMAT_SHORT_DIST1) {
						...
						case 3:
							...
							inPos++;
							dist = dist +
							       (LZMAT_GET_U4(pbIn, ref inPos, ref cur_nib) << 14)
							       + 0x4441;
							break;
						}
					}
	...
	public static byte[] DecompressOld(byte[] compressed) {
		...
		while (dstIndex < decompressedLen) {
			...
			if (partLen < 0x800000) {
				...
				partLen &= 0x7FFFFF;
				int decompressedLen2 = Lzmat_old(decompressed, dstIndex, decompressedLen - dstIndex, compressed,
					srcIndex, partLen);
