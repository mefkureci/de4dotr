...
class AssemblyDecrypter {
	static readonly string[] encryptedRequiredLocals = new[] {
		"System.Byte", "System.Byte[]", "System.Int32", "System.IO.BinaryReader", "System.IO.MemoryStream",
		"System.IO.Stream", "System.Object[]", "System.Reflection.Assembly", "System.Type[]"
	};
	...
	public bool MainAssemblyHasAssemblyResolver => InitMethod != null;
	...
	bool CheckType(TypeDef type) {
		if (DotNetUtils.GetMethod(type, "System.Byte[]",
			    "(System.Byte[],System.String,System.String,System.Int32,System.String,System.Int32)") == null)
			return false;
	...
	public byte[] Decrypt() {
		...
		byte[] encryptedData = DeobUtils.Gunzip(reader.BaseStream, reader.ReadInt32());
		reader = new BinaryReader(new MemoryStream(encryptedData));
		byte[] serializedData = reader.ReadBytes(reader.ReadInt32());
		for (int i = 0; i < serializedData.Length; i++)
			serializedData[i] ^= 0xAD;
		byte[] encryptedAssembly = reader.ReadBytes((int)(reader.BaseStream.Length - reader.BaseStream.Position));
	...
	static byte[] Decrypt(PasswordInfo password, byte[] data) {
		...
		byte[] key = new Rfc2898DeriveBytes(password.passphrase, Encoding.UTF8.GetBytes(password.salt), iterations)
			.GetBytes(numBits / 8);
	...
	public List<AssemblyInfo> GetAssemblyInfos(ISimpleDeobfuscator simpleDeobfuscator, IDeobfuscator deob) {
		...
		foreach (var rsrc in module.Resources) {
			...
			byte[] asmData = Decrypt(embedPassword, Gunzip(resource.CreateReader().ToArray()));
			var mod = ModuleDefMD.Load(asmData);
			infos.Add(new AssemblyInfo(asmData, resource, mod.Assembly.FullName, mod.Assembly.Name.String,
	...
	static PasswordInfo GetEmbedPassword(MethodDef method) {
		...
		for (int i = 0; i < instrs.Count - 3; i++) {
			...
			if (ldstr1.OpCode.Code != Code.Ldstr)
				continue;
			string passphrase = GetString(ldstr1, instrs, ref index);

			var ldstr2 = instrs[index++];
			if (ldstr2.OpCode.Code != Code.Ldstr)
				continue;
			string salt = GetString(ldstr2, instrs, ref index);
			...
			if (ldstr3.OpCode.Code != Code.Ldstr)
				continue;
			string iv = GetString(ldstr3, instrs, ref index);
	...
	public class AssemblyInfo {
		...
		public AssemblyInfo(byte[] data, EmbeddedResource resource, string asmFullName, string asmSimpleName,
			string extension) {
			this.data = data;
