...
class PasswordFinder {
	...
	static IList ToList(object obj) => (IList)obj;
	...
	public void Find(out PasswordInfo mainAsmPassword, out PasswordInfo embedPassword) {
		...
		object allTypes = serializedTypes.Deserialize(serializedData);
	...
	Obj FindType(string name) {
		foreach (object tmp in asmTypes) {
	...
	PasswordInfo FindMainAssemblyPassword() {
		...
		string passphrase = FindStringStoreValue(instrs, "KeyPassphrase");
		string salt = FindStringStoreValue(instrs, "KeySaltValue");
		string iv = FindStringStoreValue(instrs, "KeyIV");
	...
	class Obj {
		...
		public Obj MemberDef => new(ReadField("MemberDef"));
		...
		List<Obj> GetList(string name) => ConvertList((IList)ReadField(name));

		static List<Obj> ConvertList(IList inList) {
			var outList = new List<Obj>(inList.Count);
			foreach (object e in inList)
