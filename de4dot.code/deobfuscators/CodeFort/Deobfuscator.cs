...
public class DeobfuscatorInfo : DeobfuscatorInfoBase {
	...
	public override IDeobfuscator CreateDeobfuscator() =>
		new Deobfuscator(new Deobfuscator.Options {
			ValidNameRegex = validNameRegex.Get(), DumpEmbeddedAssemblies = dumpEmbeddedAssemblies.Get()
		});

	protected override IEnumerable<Option> GetOptionsInternal() =>
		new List<Option> { dumpEmbeddedAssemblies };
}
...
class Deobfuscator : DeobfuscatorBase {
	...
	public override void DeobfuscateBegin() {
		...
		staticStringInliner.Add(stringDecrypter.Method,
			(method, gim, args) => stringDecrypter.Decrypt((string)args[0]));
