...
class GenericType : ICFType {
	readonly ICFType[] genericArgs;
	readonly ICFType type;
...
class SerializedTypes {
	const string serializationAssemblyname =
		"System.Runtime.Serialization, Version=3.0.0.0, Culture=neutral, PublicKeyToken=b77a5c561934e089";

	static readonly EnumInfo[] enumInfos = new[] {
		new EnumInfo("InstructionType", "a",
			new[] {
				new EnumFieldInfo(0, "BeginCatchBlock", "1"), new EnumFieldInfo(1, "BeginExceptFilterBlock", "2"),
				new EnumFieldInfo(2, "BeginExceptionBlock", "3"), new EnumFieldInfo(3, "BeginFaultBlock", "4"),
				new EnumFieldInfo(4, "BeginFinallyBlock", "5"), new EnumFieldInfo(5, "BeginScope", "6"),
				new EnumFieldInfo(6, "LocalVariable", "7"), new EnumFieldInfo(7, "Label", "8"),
				new EnumFieldInfo(8, "NoOperand", "A"), new EnumFieldInfo(9, "ByteOperand", "B"),
				new EnumFieldInfo(10, "ConstructorOperand", "C"), new EnumFieldInfo(11, "DoubleOperand", "D"),
				new EnumFieldInfo(12, "FieldOperand", "E"), new EnumFieldInfo(13, "SingleOperand", "F"),
				new EnumFieldInfo(14, "Int32Operand", "G"), new EnumFieldInfo(15, "TargetOperand", "H"),
				new EnumFieldInfo(16, "TargetsOperand", "I"), new EnumFieldInfo(17, "LocalOperand", "J"),
				new EnumFieldInfo(18, "Int64Operand", "K"), new EnumFieldInfo(19, "MethodOperand", "L"),
				new EnumFieldInfo(20, "SByteOperand", "M"), new EnumFieldInfo(21, "Int16Operand", "N"),
				new EnumFieldInfo(22, "StringOperand", "O"), new EnumFieldInfo(23, "TypeOperand", "P"),
				new EnumFieldInfo(24, "EndExceptionBlock", "b"), new EnumFieldInfo(25, "EndScope", "c"),
				new EnumFieldInfo(26, "MarkLabel", "d"), new EnumFieldInfo(27, "NotImpl1", "e"),
				new EnumFieldInfo(28, "ThrowException", "f"), new EnumFieldInfo(29, "NotImpl2", "g")
			}),
		new EnumInfo("MemberTypes1", "l",
			new[] {
				new EnumFieldInfo(0, "Constructor", "1"), new EnumFieldInfo(1, "TypeInitializer", "7"),
				new EnumFieldInfo(2, "Method", "2"), new EnumFieldInfo(3, "Field", "3"),
				new EnumFieldInfo(4, "Property", "4"), new EnumFieldInfo(5, "Event", "5"),
				new EnumFieldInfo(6, "NestedType", "6")
			})
	};

	static readonly TypeInfo[] typeInfos = new[] {
		new TypeInfo("AllTypes", "b", new[] { new TypeFieldInfo(new ListType("TypeDef"), "Types", "T") }),
		new TypeInfo("Instruction", "c",
			new[] {
		...
		new TypeInfo("LocalVariable", "e",
			new[] {
				new TypeFieldInfo(typeof(bool), "IsPinned", "P"), new TypeFieldInfo("TypeRef", "VariableType", "T")
			}),
		new TypeInfo("TypeRef", "f",
			new[] {
		...
		new TypeInfo("AssemblyRef", "g", new[] { new TypeFieldInfo(typeof(string), "Name", "N") }),
		new TypeInfo("MemberRef", "h",
			new[] {
				new TypeFieldInfo(typeof(int), "BindingFlags", "B"), new TypeFieldInfo("TypeRef", "DeclaringType", "C"),
				new TypeFieldInfo("MemberTypes1", "MemberTypes1", "K"),
				new TypeFieldInfo("MemberDef", "MemberDef", "M"), new TypeFieldInfo(typeof(string), "Name", "N"),
				new TypeFieldInfo("TypeRef", "ReturnType", "T")
			}),
		new TypeInfo(ITypeCreator.Create("MemberRef"), "MethodRef", "i",
			new[] {
				new TypeFieldInfo(new ListType("ParameterRef"), "Parameters", "P"),
				new TypeFieldInfo(typeof(int), "CallingConventions", "V")
			}),
		new TypeInfo("ParameterRef", "j", new[] { new TypeFieldInfo("TypeRef", "TypeRef", "T") }),
		new TypeInfo("TypeDef", "k",
			new[] {
				new TypeFieldInfo(typeof(int), "TypeAttributes", "A"), new TypeFieldInfo("TypeRef", "BaseType", "B"),
		...
		new TypeInfo("MemberDef", "m",
			new[] {
				new TypeFieldInfo(typeof(int), "Attributes", "B"),
				new TypeFieldInfo("MemberTypes1", "MemberTypes1", "K"), new TypeFieldInfo(typeof(string), "Name", "N"),
				new TypeFieldInfo("TypeRef", "Type", "T")
			}),
		new TypeInfo(ITypeCreator.Create("MemberDef"), "PropertyDef", "n",
			new[] {
		...
		new TypeInfo(ITypeCreator.Create("MemberDef"), "EventDef", "o",
			new[] {
				new TypeFieldInfo("MethodDef", "AddOnMethod", "A"),
				new TypeFieldInfo("MethodDef", "RemoveOnMethod", "R")
			}),
		new TypeInfo(ITypeCreator.Create("MemberDef"), "MethodDef", "p",
			new[] {
		...
		new TypeInfo("ParameterDef", "q",
			new[] { new TypeFieldInfo(typeof(string), "Name", "N"), new TypeFieldInfo("TypeRef", "TypeRef", "T") })
	};

	readonly Dictionary<string, Type> createdTypes = new(StringComparer.Ordinal);
	Dictionary<string, EnumBuilder> enumBuilders = new(StringComparer.Ordinal);

	ModuleBuilder moduleBuilder;
	Dictionary<string, TypeBuilder> typeBuilders = new(StringComparer.Ordinal);
	...
	void InitializeTypes() {
		foreach (var info in typeInfos) {
			...
			foreach (var fieldInfo in info.fieldInfos) {
				var fieldBuilder =
					builder.DefineField(fieldInfo.name, fieldInfo.type.Get(this), FieldAttributes.Public);
	...
	public object Deserialize(byte[] data) {
		var serializerType =
			Type.GetType("System.Runtime.Serialization.DataContractSerializer," + serializationAssemblyname);
		...
		var serializerCtor = serializerType.GetConstructor(new[] { typeof(Type), typeof(IEnumerable<Type>) });
		object serializer = serializerCtor.Invoke(new object[] {
			GetType("AllTypes"),
			new[] {
				GetType("MemberTypes1"), GetType("Instruction"), GetType("InstructionType"),
				GetType("InstructionLabel"), GetType("LocalVariable"), GetType("ParameterDef"), GetType("TypeDef"),
				GetType("MemberDef"), GetType("MethodDef"), GetType("EventDef"), GetType("PropertyDef"),
				GetType("ParameterRef"), GetType("TypeRef"), GetType("MemberRef"), GetType("MethodRef")
			}
		});

		var xmlReaderType = Type.GetType("System.Xml.XmlDictionaryReader," + serializationAssemblyname);
		var createReaderMethod = xmlReaderType.GetMethod("CreateBinaryReader", new[] { typeof(Stream), quotasType });
		object xmlReader = createReaderMethod.Invoke(null,
			new[] {
				new MemoryStream(data),
				quotasType.InvokeMember("Max", BindingFlags.GetProperty, null, null, new object[0])
			});
		using ((IDisposable)xmlReader) {
			var readObjectMethod = serializerType.GetMethod("ReadObject", new[] { xmlReaderType });
			return readObjectMethod.Invoke(serializer, new[] { xmlReader });
		}
	}

	class PropertyInfoCreator {
		readonly List<PropertyInfo> properties = new();
		readonly Type type;
