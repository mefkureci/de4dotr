...
public class DeobfuscatorInfo : DeobfuscatorInfoBase {
	...
	readonly IntOption stringCodePage;
	...
	public override IDeobfuscator CreateDeobfuscator() =>
		new Deobfuscator(new Deobfuscator.Options {
			RenameResourcesInCode = false, ValidNameRegex = validNameRegex.Get(), StringCodePage = stringCodePage.Get()
		});
	...
	public override void DeobfuscateBegin() {
		...
		if (stringDecrypter.Detected) {
			...
			staticStringInliner.Add(stringDecrypter.Method,
				(method, gim, args) => stringDecrypter.Decrypt((uint)args[0]));
	...
	void RemoveInvalidResources() {
		foreach (var tmp in module.Resources) {
			...
			if (resource.Offset == null ||
			    (resource.CreateReader().StartOffset == 0 && resource.CreateReader().Length == 0))
