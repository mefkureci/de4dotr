...
class MethodsDecrypter {
	readonly DecrypterInfo decrypterInfo;
	...
	DumpedMethods DecryptMethods() {
		...
		var methodInfos = new MethodInfos(module, decrypterInfo.mainType, peImage, decrypterInfo.peHeader,
			decrypterInfo.mcKey);
	...
	void DecryptResources() {
		...
		var peImage = decrypterInfo.peImage;
		byte[] fileData = decrypterInfo.fileData;
	...
	void DecryptStrings() {
		...
		if (usHeap.StartOffset == 0 || // Start offset is 0 if it's not present in the file
		    ...
		    usHeapSize != usHeap.EndOffset - usHeap.StartOffset) {
		...
		while (usHeapOffset < usHeapEnd) {
			...
			try {
				Logger.v("Decrypted string: {0}",
					Utils.ToCsharpString(Encoding.Unicode.GetString(fileData, usHeapOffsetString,
						stringDataLength - 1)));
			}
			catch {
	...
	class MethodInfos {
		...
		static readonly byte[] decrypt3Shifts =
			new byte[16] { 5, 11, 14, 21, 6, 20, 17, 29, 4, 10, 3, 2, 7, 1, 26, 18 };

		readonly List<IDecrypter> decrypters = new();
		readonly uint encryptedDataOffset;
		readonly MainType mainType;
		readonly McKey mcKey;
		readonly uint methodInfosOffset;
		readonly ModuleDef module;
		...
		byte[] blowfishKey;
		Dictionary<uint, DecryptedMethodInfo> infos = new();
		uint xorKey;
		...
		void InitializeDecrypter() {
			...
			case EncryptionVersion.V1:
				decrypters.Add(new Decrypter(
					new DecryptFunc[] {
						Decrypt1_v1, Decrypt4_v1, Decrypt2_v1, Decrypt3_v1, Decrypt5, Decrypt6, Decrypt7
					}, new uint[] { 0x462FA2D2 }));
				decrypters.Add(new Decrypter(
					new DecryptFunc[] {
						Decrypt4_v1, Decrypt1_v1, Decrypt2_v1, Decrypt3_v1, Decrypt5, Decrypt6, Decrypt7
					}, new uint[] { 0x471299D3 }));
				break;
			...
			case EncryptionVersion.V2:
				decrypters.Add(new Decrypter(
					new DecryptFunc[] {
						Decrypt3_v1, Decrypt2_v1, Decrypt1_v1, Decrypt4_v1, Decrypt5, Decrypt6, Decrypt7
					},
					new uint[] {
						0x482384FB, 0x4A5EEC64, 0x4BD6F703, 0x4C6220EC, 0x4C622357, 0x4C6E4605, 0x4D0E220D, 0x4DC2FC75,
						0x4DC2FE0C, 0x4DFA3D5D
					})); break;
			case EncryptionVersion.V3:
				decrypters.Add(new Decrypter(
					new DecryptFunc[] {
						Decrypt1_v1, Decrypt2_v1, Decrypt3_v1, Decrypt4_v1, Decrypt5, Decrypt6, Decrypt7
					}, new uint[] { 0x4ECF2195, 0x4ED76740, 0x4EE1FAD1 })); break;
			case EncryptionVersion.V4:
				decrypters.Add(new Decrypter(
					new DecryptFunc[] {
						Decrypt2_v1, Decrypt1_v1, Decrypt3_v1, Decrypt4_v1, Decrypt5, Decrypt6, Decrypt7
					}, new uint[] { 0x4F832868, 0x4F8C86BE, 0x4F9447DB, 0x4FDEF2FF })); break;

			case EncryptionVersion.V5:
				decrypters.Add(new Decrypter(
					new DecryptFunc[] {
						Decrypt4_v1, Decrypt2_v1, Decrypt3_v1, Decrypt1_v1, Decrypt5, Decrypt6, Decrypt7
					}, new uint[] { 0x4F8E262C, 0x4F966B0B, 0x4FAB3CCF }));
				decrypters.Add(new Decrypter(
					new DecryptFunc[] {
						Decrypt4_v2, Decrypt2_v2, Decrypt3_v2, Decrypt1_v2, Decrypt6, Decrypt7, Decrypt5
					}, new uint[] { 0x4FC7459E, 0x4FCEBD7B }));
				decrypters.Add(new Decrypter(
					new DecryptFunc[] {
						Decrypt4_v3, Decrypt2_v3, Decrypt3_v3, Decrypt1_v3, Decrypt6, Decrypt7, Decrypt5
					}, new uint[] { 0x4FBE81DE }));
				break;
			...
			case EncryptionVersion.V6:
				decrypters.Add(new Decrypter(
					new DecryptFunc[] {
						Decrypt4_v4, Decrypt2_v4, Decrypt3_v4, Decrypt1_v4, Decrypt6, Decrypt7, Decrypt5
					}, new uint[] { 0x50A0963C })); break;
			case EncryptionVersion.V7:
				decrypters.Add(new Decrypter(
					new DecryptFunc[] {
						Decrypt4_v5, Decrypt2_v5, Decrypt3_v5, Decrypt1_v5, Decrypt6, Decrypt8_v5, Decrypt9_v5,
						Decrypt7, Decrypt5
					}, new uint[] { 0x50D367A5 })); break;

			case EncryptionVersion.V8:
				decrypters.Add(new Decrypter(
					new DecryptFunc[] {
						Decrypt4_v6, Decrypt2_v2, Decrypt3_v6, Decrypt1_v6, Decrypt6, Decrypt8_v6, Decrypt9_v6,
						Decrypt7, Decrypt10, Decrypt5
					}, new uint[] { 0x5166DB4F, 0x51927495 }));
				decrypters.Add(new Decrypter(
					new DecryptFunc[] {
						Decrypt4_v7, Decrypt2_v2, Decrypt3_v6, Decrypt1_v7, Decrypt6, Decrypt8_v7, Decrypt9_v7,
						Decrypt7, Decrypt5
					}, new uint[] { 0x51413D68 }));
				decrypters.Add(new Decrypter(
					new DecryptFunc[] {
						Decrypt4_v7, Decrypt2_v2, Decrypt3_v6, Decrypt1_v7, Decrypt6, Decrypt8_v8, Decrypt9_v8,
						Decrypt7, Decrypt5
					}, new uint[] { 0x513D7124, 0x51413BD8 }));
				decrypters.Add(new Decrypter(
					new DecryptFunc[] {
						Decrypt4_v5, Decrypt2_v2, Decrypt3_v6, Decrypt1_v9, Decrypt6, Decrypt8_v8, Decrypt9_v9,
						Decrypt7, Decrypt5
					}, new uint[] { 0x513D4492, 0x5113E277 }));
				decrypters.Add(new Decrypter(
					new DecryptFunc[] {
						Decrypt3_v6, Decrypt2_v2, Decrypt4_v8, Decrypt1_v10, Decrypt8_v9, Decrypt9_v10, Decrypt6,
						Decrypt7, Decrypt5
					}, new uint[] { 0x526BDD12 }));
				decrypters.Add(new Decrypter(
					new DecryptFunc[] {
						Decrypt1_v10, Decrypt4_v8, Decrypt2_v2, Decrypt3_v6, Decrypt6, Decrypt8_v9, Decrypt9_v10,
						Decrypt7, Decrypt5
					}, new uint[] { 0x526BC020 }));
				decrypters.Add(new Decrypter(
					new DecryptFunc[] {
						Decrypt3_v7, Decrypt2_v6, Decrypt4_v9, Decrypt1_v11, Decrypt8_v10, Decrypt11_v1, Decrypt6,
						Decrypt7, Decrypt5
					}, new uint[] { 0x5296E242, 0x52B3043C }));
				decrypters.Add(new Decrypter(
					new DecryptFunc[] {
						Decrypt4_v10, Decrypt1_v12, Decrypt3_v8, Decrypt2_v7, Decrypt6, Decrypt8_v11, Decrypt9_v11,
						Decrypt7, Decrypt5
					}, new uint[] { 0x531729C4 }));
				decrypters.Add(new Decrypter(
					new DecryptFunc[] {
						Decrypt1_v13, Decrypt4_v11, Decrypt2_v8, Decrypt3_v9, Decrypt6, Decrypt8_v11, Decrypt9_v12,
						Decrypt7, Decrypt5
					}, new uint[] { 0x52B2B2A3 }));
				decrypters.Add(new Decrypter(
					new DecryptFunc[] {
						Decrypt2_v9, Decrypt3_v10, Decrypt1_v10, Decrypt4_v12, Decrypt8_v12, Decrypt9_v13, Decrypt6,
						Decrypt7, Decrypt5
					}, new uint[] { 0x53172907 }));
				break;
		...
		uint GetRuntimeTimeStamp() {
			...
			try {
				...
				for (var di = new DirectoryInfo(Path.GetDirectoryName(path)); di != null; di = di.Parent) {
					foreach (string dllName in rtNames) {
						try {
			...
			foreach (var decrypter in decrypters) {
				try {
					if (InitializeInfos2(decrypter)) {
						if (savedInfos != null) {
							Logger.w(
								"Decryption probably failed. Make sure the correct MaxtoCode runtime file is present.");
							break;
						}
		...
		bool InitializeInfos2(IDecrypter decrypter) {
			...
			byte[][] encryptedDataInfos = new byte[numEncryptedDataInfos][];
		...
		byte[] Decrypt(IDecrypter decrypter, int type, uint dataOffset, uint encryptedSize, uint realSize) {
			...
			byte[] encrypted = ReadData(dataOffset, (int)encryptedSize);
			byte[] decrypted = decrypter.Decrypt(type, encrypted);
		...
		byte[] Decrypt3(byte[] encrypted, int offset) {
			...
			for (int i = 0; i < loopCount; i++) {
				...
				foreach (byte shift in decrypt3Shifts) {
					int shift1 = 32 - shift;
		...
		byte[] Decrypt4(byte[] encrypted, int keyStart, int keyReset, int keyEnd) {
			byte[] decrypted = new byte[encrypted.Length / 3 * 2 + 1];
		...
		class Decrypter : IDecrypter {
			readonly DecryptFunc[] decrypterHandlers;
			readonly uint[] timeStamps;
