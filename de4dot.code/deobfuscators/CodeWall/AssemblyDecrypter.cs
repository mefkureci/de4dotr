...
class AssemblyDecrypter {
	static readonly string[] requiredLocals = new[] { "System.AppDomain", "System.DateTime" };

	readonly List<AssemblyInfo> assemblyInfos = new();
	readonly IDeobfuscator deob;
	...
	public void Find() {
		...
		string mainKey = GetMainResourceKey(method, out var decryptAssemblyMethod);
		if (mainKey == null)
			return;
	...
	string GetMainResourceKey(MethodDef method, out MethodDef decryptAssemblyMethod) {
		foreach (var calledMethod in DotNetUtils.GetCalledMethods(module, method)) {
			...
			if (keyInfo == null)
				continue;
			return BitConverter
				.ToString(new MD5CryptoServiceProvider().ComputeHash(new ASCIIEncoding().GetBytes(keyInfo)))
				.Replace("-", "");
		}
	...
	ModuleDefMD GetResourceModule(string name) {
		try {
			string resourceDllFileName = Path.Combine(Path.GetDirectoryName(module.Location), name.Substring(1));
	...
	void DecryptAllAssemblies() {
		...
		foreach (var resourceElement in resourceSet.ResourceElements) {
			...
			byte[] assemblyData = Decrypt((byte[])resourceData.Data);
			var theModule = ModuleDefMD.Load(assemblyData);
			bool isMain = resourceElement.Name == entryPointAssemblyKey;
			assemblyInfos.Add(new AssemblyInfo(assemblyData, DeobUtils.GetExtension(theModule.Kind),
				theModule.Assembly.FullName, theModule.Assembly.Name.String, isMain));
		}
	}

	byte[] Decrypt(byte[] encrypted) {
		var keyGenerator = new PasswordDeriveBytes(resourcePassword, Encoding.ASCII.GetBytes(resourceSalt));
		return DeobUtils.Inflate(DeobUtils.AesDecrypt(encrypted, keyGenerator.GetBytes(32), keyGenerator.GetBytes(16)),
			false);
	}
	...
	public class AssemblyInfo {
		...
		public AssemblyInfo(byte[] data, string extension, string assemblyFullName, string assemblySimpleName,
