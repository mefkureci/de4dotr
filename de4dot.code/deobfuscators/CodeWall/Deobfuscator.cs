...
public class DeobfuscatorInfo : DeobfuscatorInfoBase {
	...
	protected override IEnumerable<Option> GetOptionsInternal() =>
		new List<Option> { dumpEmbeddedAssemblies, decryptMainAsm };
}
...
class Deobfuscator : DeobfuscatorBase {
	...
	void InitializeStringDecrypter() {
		...
		foreach (var info in stringDecrypter.Infos)
			staticStringInliner.Add(info.Method,
				(method, gim, args) => stringDecrypter.Decrypt(method, (int)args[0], (int)args[1], (int)args[2]));
	...
	void DumpEmbeddedAssemblies() {
		...
		foreach (var info in assemblyDecrypter.AssemblyInfos) {
			string asmName = info.assemblySimpleName;
