...
public class ConstantsReader {
	...
	protected Dictionary<Local, double> localsValuesDouble = new();
	protected Dictionary<Local, int> localsValuesInt32 = new();
	protected Dictionary<Local, long> localsValuesInt64 = new();
	...
	ConstantsReader(IInstructions instructions, bool emulateConvInstrs) {
		...
		this.EmulateConvInstructions = emulateConvInstrs;
	}
	...
	public bool EmulateConvInstructions { get; set; }
	...
	public bool GetInt32(ref int index, out int val) {
		...
		for (; index < instructions.Count; index++) {
			...
			case Code.Conv_I1:
				if (!EmulateConvInstructions || stack.Count < 1)
					goto done;
