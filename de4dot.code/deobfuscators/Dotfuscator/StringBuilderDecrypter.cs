...
public class StringBuilderDecrypter {
	...
	static void GetStringBuilderFixIndexs(IList<Instruction> instructions, out List<int> nopIdxs,
		out Dictionary<int, string> ldstrIdxs) {
		...
		for (int i = 1; i < insNoNops.Count; i++) {
			if (insNoNops[i - 1].IsLdcI4() &&
			    insNoNops[i].OpCode == OpCodes.Newarr &&
			    insNoNops[i].Operand is TypeRef typeRef &&
			    typeRef.FullName == typeof(int).FullName) {
				int[] data = null;
				int index = 0;
				int arrLength = insNoNops[i - 1].GetLdcI4Value();
				if (arrLength == 1) {
