	...
	void GetFixIndexs2(MethodDef method, ref List<int> nopIdxs) {
		...
		for (int i = 0; i < instructions.Count; i++) {
			...
			if (ldtoken.OpCode.Code != Code.Ldtoken)
				continue;
			if (ldtoken.Operand is not FieldDef arrayInitField || arrayInitField.InitialValue == null ||
			...
			if (!DotNetUtils.IsMethod(calledMethod, "System.Void", "(System.Array,System.RuntimeFieldHandle)"))
				continue;
			byte[] array = arrayInitField.InitialValue;
			if (array.Length % 2 != 0)
				continue;
			for (int j = 0; j < array.Length; j += 2)
				shortArray.Add((short)(array[j] | (array[j + 1] << 8)));

			int startIndex = instructions.IndexOf(ldci4);
