...
class StringDecrypter {
	...
	readonly MethodDefAndDeclaringTypeDict<StringDecrypterInfo> stringDecrypterMethods = new();
	...
	void FindStringDecrypterMethods(TypeDef type, ISimpleDeobfuscator simpleDeobfuscator) {
		foreach (var method in DotNetUtils.FindMethods(type.Methods, "System.String",
			...
			for (int i = 0; i < instrs.Count - 6; i++) {
				...
				Logger.v("Found string decrypter method: {0}, magic: 0x{1:X8}", Utils.RemoveNewlines(info.method),
					info.magic);
				break;
			}
		}
