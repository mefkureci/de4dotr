...
public class DeobfuscatorInfo : DeobfuscatorInfoBase {
	...
	const string DEFAULT_REGEX = @"!^(?:eval_)?[a-z][a-z0-9]{0,2}$&!^A_[0-9]+$&" +
	                             @"^[\u2E80-\u8FFFa-zA-Z_<{$][\u2E80-\u8FFFa-zA-Z_0-9<>{}$.`-]*$";
	...
	protected override IEnumerable<Option> GetOptionsInternal() =>
		new List<Option> { inlineMethods, removeInlinedMethods };
}

class Deobfuscator : DeobfuscatorBase {
	readonly Options options;
	bool foundDotfuscatorAttribute;
	...
	void InitializeVersion(TypeDef attr) {
		...
		var val = Regex.Match(s, @"^(\d+(?::\d+)*\.\d+(?:\.\d+)*)$");
		if (val.Groups.Count < 2)
			return;
		obfuscatorName = "Dotfuscator " + val.Groups[1];
	}
	...
	public override void DeobfuscateBegin() {
		...
		foreach (var info in stringDecrypter.StringDecrypterInfos)
			staticStringInliner.Add(info.method,
				(method, gim, args) => stringDecrypter.Decrypt(method, (string)args[0], (int)args[1]));
