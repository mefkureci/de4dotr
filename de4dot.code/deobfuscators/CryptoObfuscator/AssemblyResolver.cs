...
class AssemblyResolver {
	...
	public List<AssemblyInfo> AssemblyInfos { get; private set; } = new();
	...
	bool CheckInitMethod(MethodDef initMethod) {
		...
		for (int i = 0; i < instructions.Count; i++) {
			...
			if (methodRef == null || methodRef.FullName !=
	...
	List<AssemblyInfo> InitializeEmbeddedAssemblies(string s) {
		...
		foreach (char c in s)
			sb.Append((char)~c);
		string[] tmpAssemblyInfos = sb.ToString().Split(new[] { asmSeparator }, StringSplitOptions.RemoveEmptyEntries);
		...
		for (int i = 0; i < tmpAssemblyInfos.Length; i += 2) {
			string assemblyName = tmpAssemblyInfos[i];
			string resourceName = tmpAssemblyInfos[i + 1];
