...
class ProxyCallFixer : ProxyCallFixer2 {
	readonly Dictionary<MethodDef, ProxyCreatorType> methodToType = new();
	...
	protected override object CheckCctor(TypeDef type, MethodDef cctor) {
		...
		for (int i = 0; i < instructions.Count; i++) {
			var instrs = DotNetUtils.GetInstructions(instructions, i, OpCodes.Ldc_I4, OpCodes.Ldc_I4, OpCodes.Ldc_I4,
				OpCodes.Call);
			if (instrs == null)
				continue;
	...
	MethodDef GetProxyCreateMethod(TypeDef type) {
		...
		foreach (var m in type.Methods) {
			if (m.Name == ".ctor" || m.Name == ".cctor")
				continue;
			if (createMethod == null &&
			    DotNetUtils.IsMethod(m, "System.Void", "(System.Int32,System.Int32,System.Int32)"))
				createMethod = m;
		}
