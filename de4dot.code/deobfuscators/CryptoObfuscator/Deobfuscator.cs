...
public class DeobfuscatorInfo : DeobfuscatorInfoBase {
	...
	const string DEFAULT_REGEX =
		@"!^(get_|set_|add_|remove_)?[A-Z]{1,3}(?:`\d+)?$&!^(get_|set_|add_|remove_)?c[0-9a-f]{32}(?:`\d+)?$&" +
		DeobfuscatorBase.DEFAULT_ASIAN_VALID_NAME_REGEX;

	readonly BoolOption decryptConstants;
	readonly BoolOption fixLdnull;
	readonly BoolOption inlineMethods;
	readonly BoolOption removeTamperProtection;
	...
	protected override IEnumerable<Option> GetOptionsInternal() =>
		new List<Option> { removeTamperProtection, decryptConstants, inlineMethods, fixLdnull };
}
...
class Deobfuscator : DeobfuscatorBase {
	...
	bool foundCryptoObfuscatorAttribute;
	bool foundObfuscatedSymbols;
	bool foundObfuscatorUserString;
	...
	protected override void ScanForObfuscator() {
		...
		foundObfuscatorUserString = Utils.StartsWith(module.ReadUserString(0x70000001),
			"\u0011\"3D9B94A98B-76A8-4810-B1A0-4BE7C4F9C98D", StringComparison.Ordinal);
	}

	void InitializeVersion(TypeDef attr) {
		string s = DotNetUtils.GetCustomArgAsString(GetAssemblyAttribute(attr), 0);
		if (s == null)
			return;
	...
	bool CheckCryptoObfuscator() {
		...
		foreach (var type in module.Types) {
			...
			if (Regex.IsMatch(type.Name.String, "^c[0-9a-f]{32}$"))
				return true;
			if (Regex.IsMatch(type.Name.String, "^A[A-Z]*$")) {
				if (++matched >= 10)
					return true;
			}
		}
	...
	public override void DeobfuscateBegin() {
		...
		if (options.DecryptConstants) {
			...
			int32ValueInliner.Add(constantsDecrypter.Int32Decrypter,
				(method, gim, args) => constantsDecrypter.DecryptInt32((int)args[0]));
			int64ValueInliner = new Int64ValueInliner();
			int64ValueInliner.Add(constantsDecrypter.Int64Decrypter,
				(method, gim, args) => constantsDecrypter.DecryptInt64((int)args[0]));
			singleValueInliner = new SingleValueInliner();
			singleValueInliner.Add(constantsDecrypter.SingleDecrypter,
				(method, gim, args) => constantsDecrypter.DecryptSingle((int)args[0]));
			doubleValueInliner = new DoubleValueInliner();
			doubleValueInliner.Add(constantsDecrypter.DoubleDecrypter,
				(method, gim, args) => constantsDecrypter.DecryptDouble((int)args[0]));
	...
	void DumpEmbeddedFile(EmbeddedResource resource, string assemblyName, string extension, string reason) {
		DeobfuscatedFile.CreateAssemblyFile(resourceDecrypter.Decrypt(resource.CreateReader().AsStream()),
			Utils.GetAssemblySimpleName(assemblyName), extension);
