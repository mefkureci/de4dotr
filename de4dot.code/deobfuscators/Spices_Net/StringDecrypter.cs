...
class StringDecrypter {
	readonly MethodDefAndDeclaringTypeDict<DecrypterInfo> methodToInfo = new();
	...
	bool CheckCctor(MethodDef cctor, out FieldDef compressedDataField, out StringDataFlags flags) {
		...
		for (int i = 0; i < instructions.Count; i++) {
			...
			var instrs = DotNetUtils.GetInstructions(instructions, i + 1, OpCodes.Newarr, OpCodes.Dup, OpCodes.Ldtoken,
				OpCodes.Call);
			if (instrs == null)
				continue;
	...
	public void Initialize() {
		...
		if ((stringDataFlags & StringDataFlags.Encrypted2) != 0) {
			byte[] k = module.Assembly.PublicKey.Data;
