...
class ResourceNamesRestorer {
	readonly Dictionary<TypeDef, bool> callsResourceManager = new();
	readonly MethodDefAndDeclaringTypeDict<IMethod> componentManagerCtors = new();
	readonly ModuleDefMD module;
	readonly MethodDefAndDeclaringTypeDict<IMethod> resourceManagerCtors = new();
	public ResourceNamesRestorer(ModuleDefMD module) => this.module = module;

	public TypeDef ResourceManagerType { get; private set; }

	public TypeDef ComponentResourceManagerType { get; private set; }
	...
	static bool Rename(ResourceDictionary rsrcDict, string ns, string name) {
		string resourceName = name + ".resources";
	...
	class ResourceDictionary {
		readonly Dictionary<Key, Resource> resources = new();
		...
		public bool Add(Resource resource) {
			...
			if (!uint.TryParse(ext, out uint extNum))
				return false;
			string ns = index < 0 ? string.Empty : name.Substring(0, index);
