...
class CastDeobfuscator : IBlocksDeobfuscator {
	readonly Dictionary<Local, LocalInfo> localInfos = new();
	Blocks blocks;
		...
		if (modified) {
			foreach (var block in allBlocks) {
				...
				for (int i = 0; i < instrs.Count - 1; i++) {
					...
					if (instr.IsLdarg())
						AddCast(block, castIndex, i + 1,
							instr.Instruction.GetArgumentType(blocks.Method.MethodSig, blocks.Method.DeclaringType));
