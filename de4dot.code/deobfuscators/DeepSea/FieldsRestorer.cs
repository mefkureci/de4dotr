...
class FieldsRestorer {
	...
	readonly FieldDefAndDeclaringTypeDict<bool> structFieldsToFix = new();
	readonly TypeDefDict<List<TypeDef>> structToOwners = new();
	readonly TypeDefDict<FieldDefAndDeclaringTypeDict<FieldDef>> typeToFieldsDict = new();
		...
		foreach (var kv in GetMovedTypes()) {
			...
			foreach (var ownerType in kv.Value) {
				...
				foreach (var structField in structType.Fields) {
					var newField = module.UpdateRowId(new FieldDefUser(structField.Name, structField.FieldSig.Clone(),
	...
	static bool CheckBaseType(TypeDef type) {
		if (type == null || type.BaseType == null)
			return false;
		string fn = type.BaseType.FullName;
	...
	static bool CheckMethods(TypeDef type) {
		foreach (var method in type.Methods) {
			if (method.Name == ".cctor")
				continue;
			if (type.BaseType != null && type.BaseType.FullName == "System.Object" && method.Name == ".ctor" &&
