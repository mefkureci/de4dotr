...
class StringDecrypter {
	...
	readonly MethodDefAndDeclaringTypeDict<IDecrypterInfo> methodToInfo = new();
	...
	public DecrypterVersion Version { get; private set; } = DecrypterVersion.Unknown;
	...
	static short[] FindKey(MethodDef initMethod, FieldDefAndDeclaringTypeDict<bool> fields) {
		...
		for (int i = 0; i < instrs.Count - 2; i++) {
			...
			i++;
			short[] array = ArrayFinder.GetInitializedInt16Array(ldci4.GetLdcI4Value(), initMethod, ref i);
			if (array == null)
				continue;
	...
	static void DeobfuscateCctor(ISimpleDeobfuscator simpleDeobfuscator, MethodDef cctor, ref bool deobfuscatedCctor,
	...
	class DecrypterInfo41 : IDecrypterInfo {
		...
		public bool Initialize() {
			...
			Buffer.BlockCopy(arrayInfo.initField.InitialValue, 0, encryptedData, 0,
				arrayInfo.initField.InitialValue.Length);
		...
		ArrayInfo GetArrayInfo(MethodDef method) {
			...
			for (int i = 0; i < instructions.Count; i++) {
				...
				if (!ldci4_arraySizeInBytes.IsLdcI4())
					continue;
				i++;
				var instrs = DotNetUtils.GetInstructions(instructions, i, OpCodes.Newarr, OpCodes.Dup, OpCodes.Ldtoken,
					OpCodes.Call, OpCodes.Stsfld);
				if (instrs == null)
					continue;
		...
		short[] GetPublicKeyTokenKey(byte[] publicKeyToken) {
			...
			short[] key = new short[publicKeyToken.Length];
		...
		string DecryptTrial(int magic2, int magic3) {
			...
			ushort keyChar = encryptedData[offset + 1];
		...
		string DecryptRetail(int magic2, int magic3, int keyCharOffs, int cachedIndexOffs, int flagsOffset, int flag,
			int keyDispl) {
			int offset = magic ^ magic2 ^ magic3;
			ushort keyChar = encryptedData[offset + keyCharOffs];
			...
			var sb = new StringBuilder(numChars);
			for (int i = 0; i < numChars; i++)
				sb.Append((char)(keyChar ^ encryptedData[offset + numChars - i - 1] ^
				                 key[(i + 1 + keyDispl + offset) % key.Length]));
			return sb.ToString();
		}
		...
		class ArrayInfo {
			public readonly FieldDef initField;
	...
	class DecrypterInfo40 : IDecrypterInfo {
		...
		public bool Initialize() {
			...
			Buffer.BlockCopy(encryptedDataField.InitialValue, 0, encryptedData, 0,
				encryptedDataField.InitialValue.Length);
		...
		static FieldDef FindEncryptedStrings(MethodDef initMethod, List<FieldDef> ourFields, out FieldDef dataField) {
			for (int i = 0; i < initMethod.Body.Instructions.Count; i++) {
				var instrs = DotNetUtils.GetInstructions(initMethod.Body.Instructions, i, OpCodes.Ldtoken, OpCodes.Call,
					OpCodes.Stsfld);
				if (instrs == null)
					continue;
		...
		string Decrypt(int magic2, int magic3) {
			...
			int stringLen = encryptedData[index++] + (encryptedData[index++] << 16);
	...
	class DecrypterInfo13 : IDecrypterInfo {
		...
		string Decrypt(int magic2) {
			string es = encryptedStrings[magic ^ magic2];
