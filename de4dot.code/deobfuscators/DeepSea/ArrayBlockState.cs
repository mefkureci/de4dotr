...
class ArrayBlockState {
	readonly FieldDefAndDeclaringTypeDict<FieldInfo> fieldToInfo = new();
	...
	bool InitializeArrays2(ISimpleDeobfuscator simpleDeobfuscator, MethodDef method) {
		...
		for (int i = 0; i < instructions.Count; i++) {
			...
			if (arrayInitField == null || arrayInitField.InitialValue == null ||
			    arrayInitField.InitialValue.Length == 0)
				continue;
	...
	public class FieldInfo {
		...
		static Array CreateArray(ElementType etype, byte[] data) {
			...
			case ElementType.I2:
			case ElementType.U2:
				ushort[] ary2 = new ushort[data.Length / 2];
				Buffer.BlockCopy(data, 0, ary2, 0, ary2.Length * 2);
				return ary2;

			case ElementType.I4:
			case ElementType.U4:
				uint[] ary4 = new uint[data.Length / 4];
