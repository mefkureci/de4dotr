...
class DsMethodCallInliner : MethodCallInlinerBase {
	readonly CachedCflowDeobfuscator cflowDeobfuscator;
	readonly InstructionEmulator instructionEmulator = new();
	Parameter arg1, arg2;
	...
	bool InlineMethod(MethodDef methodToInline, int instrIndex, int const1, int const2) {
		...
		if (!EmulateInstructions(ref index, false))
			return false;
		var patcher = TryInlineOtherMethod(instrIndex, methodToInline, methodToInline.Body.Instructions[index],
			index + 1, 2);
		if (patcher == null)
			return false;
