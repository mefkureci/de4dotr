...
class ImageReader {
	static readonly int METHODS_SIG = 0x0000BEBA;
	static readonly int METADATA_SIG = 0x0100BEBA;
	static readonly int METHOD_NAMES_SIG = 0x0200BEBA;
	static readonly int ASSEMBLY_NAMES_SIG = 0x0201BEBA;
	static readonly int TYPEREFS_SIG = 0x0202BEBA;
	static readonly int STRINGS_SIG = 0x0203BEBA;
	readonly MemberRefConverter memberRefConverter;
	...
	public IField ReadFieldRef() {
		...
		if (fields == null || fields.Count != 1)
			throw new ApplicationException(
				$"Couldn't find one field named '{name}' in type {Utils.RemoveNewlines(declaringType)}");
	...
	public IMethod ReadMethodRef() {
		...
		if (method == null)
			throw new ApplicationException(
				$"Could not find method '{Utils.RemoveNewlines(babelMethodRef.Name)}' in type '{Utils.RemoveNewlines(babelMethodRef.DeclaringType)}'");

		var git = babelMethodRef.DeclaringType.ToGenericInstSig();
		if (git == null)
			return method;

		var mr = new MemberRefUser(module, method.Name, method.MethodSig.Clone(),
			babelMethodRef.DeclaringType.ToTypeDefOrRef());
	...
	IMethod GetMethodRef(BabelMethodreference babelMethodRef) {
		...
		if (methods.Count != 1)
			throw new ApplicationException(
				$"Couldn't find one method named '{babelMethodRef.Name}' in type {Utils.RemoveNewlines(declaringType)}");
	...
	static CallingConvention ConvertCallingConvention(CR.CallingConvention callingConvention) {
		switch (callingConvention) {
		case CR.CallingConvention.Winapi: return CallingConvention.Default;
		case CR.CallingConvention.Cdecl: return CallingConvention.C;
		case CR.CallingConvention.StdCall: return CallingConvention.StdCall;
		case CR.CallingConvention.ThisCall: return CallingConvention.ThisCall;
		case CR.CallingConvention.FastCall: return CallingConvention.FastCall;
	...
	void InitializeTypeRefs(int headerOffset) {
		...
		foreach (var kv in genericArgFixes) {
			var git = kv.Key;
			foreach (int typeNum in kv.Value)
	...
	PtrSig ReadPointerType() => new(ReadTypeSig());
	...
	ByRefSig ReadByRefType() => new(ReadTypeSig());
