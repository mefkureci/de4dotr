...
class ResourceDecrypterCreator {
	...
	public ResourceDecrypter Create() => new(module, simpleDeobfuscator);
}
...
class ResourceDecrypter {
	...
	class Decrypter1 : IDecrypter {
		...
		public byte[] Decrypt(byte[] encryptedData) {
			...
			bool isCompressed = GetHeaderData(reader, out byte[] key, out byte[] iv);
			byte[] data = DeobUtils.DesDecrypt(encryptedData,
	...
	class Decrypter2 : IDecrypter {
		...
		public byte[] Decrypt(byte[] encryptedData) {
			int index = 0;
			bool isCompressed = GetKeyIv(GetHeaderData(encryptedData, ref index), out byte[] key, out byte[] iv);
			byte[] data = DeobUtils.DesDecrypt(encryptedData, index, encryptedData.Length - index, key, iv);
			if (isCompressed)
				data = DeobUtils.Inflate(data, true);
			return data;
		}
		...
		byte[] GetHeaderData(byte[] encryptedData, ref int index) {
			...
			byte[] headerData = new byte[BitConverter.ToUInt16(encryptedData, index)];
