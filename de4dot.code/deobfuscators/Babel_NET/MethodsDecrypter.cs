...
class MethodsDecrypter {
	...
	readonly Dictionary<string, ImageReader> imageReaders = new(StringComparer.Ordinal);
	...
	public MethodsDecrypter(ModuleDefMD module, ResourceDecrypter resourceDecrypter,
	...
	public void Find() {
		string[] requiredFields = new[] { "System.Threading.ReaderWriterLock", "System.Collections.Hashtable" };
	...
	ImageReader CreateImageReader(string feature) {
		...
		try {
			byte[] encrypted = File.ReadAllBytes(GetFile(Path.GetDirectoryName(module.Location), feature));
			byte[] decrypted = resourceDecrypter.Decrypt(encrypted);
	...
	class EncryptInfo {
		public readonly string encryptedMethodName;
