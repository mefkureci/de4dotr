...
class StringDecrypter {
	...
	IDecrypterInfo CheckNested(TypeDef type, TypeDef nested) {
		...
		if (nested.Fields.Count == 1 || nested.Fields.Count == 3) {
			...
			var decrypterBuilderMethod = DotNetUtils.GetMethod(nested, "System.Reflection.Emit.MethodBuilder",
			...
			return new DecrypterInfoV3(resourceDecrypter) {
				Decrypter = decrypter, OffsetCalcInstructions = GetOffsetCalcInstructions(decrypterBuilderMethod)
			};
		}

		if (nested.Fields.Count == 2) {
			// 3.0 - 3.5
	...
	public void Initialize() {
		...
		if (decrypterInfo.NeedsResource) {
			Resource = BabelUtils.FindEmbeddedResource(module, Type);
			if (Resource == null)
				return;
		}

		decrypterInfo.Initialize(module, Resource);
	}
	...
	class DecrypterInfoV3 : IDecrypterInfo {
		readonly InstructionEmulator emulator = new();
		readonly Dictionary<int, string> offsetToString = new();
		readonly ResourceDecrypter resourceDecrypter;
		...
		public void Initialize(ModuleDefMD module, EmbeddedResource resource) {
			byte[] decrypted = resourceDecrypter.Decrypt(resource.CreateReader().ToArray());
	...
	class ReflectionToDNLibMethodCreator {
		readonly InstructionEmulator emulator;
		...
		bool DoCall(Instruction instr) {
			...
			string fn = calledMethod.FullName;
			...
			if (fn == "System.String System.Text.Encoding::GetString(System.Byte[])") {
				emulator.Push(new StringValue(Encoding.UTF8.GetString((byte[])((UserValue)emulator.Pop()).obj)));
				return true;
			}

			if (fn == "System.Int32 System.Int32::Parse(System.String)") {
				emulator.Push(new Int32Value(int.Parse(((StringValue)emulator.Pop()).value)));
				return true;
			}

			if (fn == "System.String[] System.String::Split(System.Char[])") {
				char[] ary = (char[])((UserValue)emulator.Pop()).obj;
				string s = ((StringValue)emulator.Pop()).value;
				emulator.Push(new UserValue(s.Split(ary)));
				return true;
			}

			if (sig != null && sig.HasThis &&
			    calledMethod.DeclaringType.FullName == "System.Reflection.Emit.ILGenerator" &&
			    calledMethod.Name == "Emit") {
				Value operand = null;
				...
				AddInstruction(new Instruction { OpCode = opcode, Operand = CreateDNLibOperand(opcode, operand) });
				return true;
			}
